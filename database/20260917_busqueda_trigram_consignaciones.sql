...
CREATE INDEX IF NOT EXISTS idx_trgm_consignaciones_ventas ON consignaciones_ventas USING gin (f_unaccent(COALESCE(serie, '') || '-' || COALESCE(secuencial, '') || ' ' || COALESCE(observaciones, '') || ' ' || COALESCE(punto_partida, '') || ' ' || COALESCE(punto_llegada, '') || ' ' || COALESCE(lpad(extract(day FROM fecha_emision)::int::text, 2, '0') || '-' || lpad(extract(month FROM fecha_emision)::int::text, 2, '0') || '-' || lpad(extract(year FROM fecha_emision)::int::text, 4, '0'), '') || ' ' || COALESCE(lpad(extract(year FROM fecha_emision)::int::text, 4, '0') || '-' || lpad(extract(month FROM fecha_emision)::int::text, 2, '0') || '-' || lpad(extract(day FROM fecha_emision)::int::text, 2, '0'), '') || ' ' || COALESCE(total::text, '')) gin_trgm_ops);
