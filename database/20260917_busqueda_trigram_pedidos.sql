...
CREATE INDEX IF NOT EXISTS idx_trgm_pedidos_cabecera ON pedidos_cabecera USING gin (f_unaccent(COALESCE(establecimiento, '') || '-' || COALESCE(punto_emision, '') || '-' || LPAD(COALESCE(secuencial, ''), GREATEST(9, LENGTH(COALESCE(secuencial, ''))), '0') || ' ' || COALESCE(observaciones, '') || ' ' || COALESCE(observaciones_internas, '') || ' ' || COALESCE(lpad(extract(year FROM fecha_pedido)::int::text, 4, '0') || '-' || lpad(extract(month FROM fecha_pedido)::int::text, 2, '0') || '-' || lpad(extract(day FROM fecha_pedido)::int::text, 2, '0'), '') || ' ' || COALESCE(lpad(extract(year FROM fecha_entrega)::int::text, 4, '0') || '-' || lpad(extract(month FROM fecha_entrega)::int::text, 2, '0') || '-' || lpad(extract(day FROM fecha_entrega)::int::text, 2, '0'), '') || ' ' || COALESCE(lpad(extract(hour FROM hora_inicial_entrega)::int::text, 2, '0') || ':' || lpad(extract(minute FROM hora_inicial_entrega)::int::text, 2, '0'), '') || ' - ' || COALESCE(lpad(extract(hour FROM hora_maxima_entrega)::int::text, 2, '0') || ':' || lpad(extract(minute FROM hora_maxima_entrega)::int::text, 2, '0'), '')) gin_trgm_ops);
