...
-- VARIANTE B: la subconsulta correlacionada de tipo_ambiente reemplazada por el
-- literal '2' (el ambiente real de la empresa 30). Todo lo demas es identico.
-- Sirve para comprobar si esa subconsulta es la que rompe la estimacion de filas.
-- Ejecutar tal cual en pgAdmin. Al final del resultado: "Execution Time".
--
...
EXPLAIN (ANALYZE, BUFFERS)
            ...
            SELECT l.fecha_emision + INTERVAL '1 day' *
                ...
                WHERE l.id_empresa    IN (30)
                  ...
                  AND (l.tipo_ambiente IS NULL OR l.tipo_ambiente = '2')
            ...
            + INTERVAL '1 day' * COALESCE(fe.plazo_dias, 0)
                ...
                WHERE fe.eliminado    = false
                  ...
                  AND ic.id_empresa   IN (30)
                  AND ic.tipo_ambiente = '2'
            )
            SELECT
                d.*,
