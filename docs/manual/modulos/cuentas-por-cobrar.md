...
día; la vista *Por producto* no las lleva) y la tabla. Si los filtros no
devuelven ningún documento, la tabla sale con sus encabezados y una línea que lo
dice —*No se encontraron cuentas por cobrar con los filtros aplicados*—, en vez
de una hoja con las tarjetas en cero y nada debajo.

La hoja es **A4 vertical** y la tabla sale con letra de **8 puntos** (8.5 en los
encabezados de columna y en el título del recuadro de filtros), un tamaño pensado
para que las 8-10 columnas entren en el ancho de la hoja sin recortar nada: lo que
no cabe en una celda pasa a la línea de abajo, no se corta.

Cada hoja lleva al pie, a la derecha, su **número de página** sobre el total
(*Página 2/5*), en las tres vistas, para ordenar el reporte impreso o citar una
hoja concreta.
...
imprimir. Debajo del nombre de cada cliente se repite el **encabezado de
columnas** (Fecha, N. Documento, Total, …), así que se puede leer cualquier
sección sin subir a buscar el rótulo; además, ese mismo encabezado aparece al
principio de cada página para los clientes cuyos documentos siguen en la hoja
siguiente.
...
- **2.27** — Los botones y enlaces de **PDF** de los documentos preguntan ahora si se quiere
...
- **2.23** — Los **PDF del listado** (vistas *Detallado*, *Por cliente* y *Por
  ...
  Actualizada *El PDF del listado*.

- **2.22** — En el PDF **Por cliente**, el **encabezado de columnas vuelve a
  repetirse bajo cada cliente**, además de seguir apareciendo al principio de
  cada página. Al unificar el listado en una sola tabla (2.20) el rótulo había
  quedado solo al tope de la hoja, así que un cliente que empezaba a media página
  mostraba sus documentos sin saber a qué columna correspondía cada valor. El
  primer cliente no lo repite: el encabezado de la tabla está justo encima.
  Actualizada *Vista "Por cliente"*.

- **2.21** — El **PDF de la vista *Detallado*** ya avisa cuando el filtro no
  devuelve nada: antes salía con las tarjetas en cero, sin encabezados de tabla y
  sin explicación; ahora imprime la cabecera de columnas y la línea *No se
...
- **2.18** — **El centavo pendiente ya se puede cobrar.** El saldo de
  ...
  corrigió fue Ingresos.

- **2.17** — Los **PDF del módulo se leen más grandes**: la tabla pasa de 7 a
  **8 puntos** y los encabezados de columna de 7.5 a **8.5**; el recuadro
  *Filtros aplicados* queda en 8 puntos, con su título en 8.5. Las columnas
  siguen entrando en el A4 vertical (un listado de 250 documentos ocupa una
  página más que antes). Actualizada *El PDF del listado*.
