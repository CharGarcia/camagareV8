...
if (mysqli_num_rows($resultado_productos) > 0) {
	...
	$titulosColumnas = array('Código', 'Producto', 'Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre', 'Total unidades');
	...
	while ($row = mysqli_fetch_array($resultado_productos)) {
		...
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('D' . $i,  $row_result['cantidad_feb'] > 0 ? number_format($row_result['cantidad_feb'], $decimal, '.', '') : "")
			->setCellValue('E' . $i,  $row_result['cantidad_mar'] > 0 ? number_format($row_result['cantidad_mar'], $decimal, '.', '') : "")
			->setCellValue('F' . $i,  $row_result['cantidad_abr'] > 0 ? number_format($row_result['cantidad_abr'], $decimal, '.', '') : "")
			->setCellValue('G' . $i,  $row_result['cantidad_may'] > 0 ? number_format($row_result['cantidad_may'], $decimal, '.', '') : "")
			->setCellValue('H' . $i,  $row_result['cantidad_jun'] > 0 ? number_format($row_result['cantidad_jun'], $decimal, '.', '') : "")
			->setCellValue('I' . $i,  $row_result['cantidad_jul'] > 0 ? number_format($row_result['cantidad_jul'], $decimal, '.', '') : "")
			->setCellValue('J' . $i,  $row_result['cantidad_ago'] > 0 ? number_format($row_result['cantidad_ago'], $decimal, '.', '') : "")
			->setCellValue('K' . $i,  $row_result['cantidad_sep'] > 0 ? number_format($row_result['cantidad_sep'], $decimal, '.', '') : "")
			->setCellValue('L' . $i,  $row_result['cantidad_oct'] > 0 ? number_format($row_result['cantidad_oct'], $decimal, '.', '') : "")
			->setCellValue('M' . $i,  $row_result['cantidad_nov'] > 0 ? number_format($row_result['cantidad_nov'], $decimal, '.', '') : "")
			->setCellValue('N' . $i,  $row_result['cantidad_dic'] > 0 ? number_format($row_result['cantidad_dic'], $decimal, '.', '') : "")
			->setCellValue('O' . $i,  number_format($suma_total, $decimal, '.', ''));
		$i++;
	}
	...
	header('Content-Disposition: attachment;filename="ReportVtaUnidades.xlsx"');
