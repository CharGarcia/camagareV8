...
if (isset($_SESSION['id_usuario']) && isset($_SESSION['id_empresa']) && isset($_SESSION['ruc_empresa'])) {
	...
	$query_pasa_datos_proforma = mysqli_query($con, "INSERT INTO factura_tmp(id, id_producto, cantidad_tmp, precio_tmp, descuento, tipo_produccion, tarifa_iva, tarifa_ice, tarifa_botellas, id_usuario, id_bodega, id_medida, lote, vencimiento, subtotal, ruc_empresa) 
SELECT null,id_producto,cantidad,valor_unitario, descuento,tipo_produccion,tarifa_iva,tarifa_ice,tarifa_bp,'" . $id_usuario . "',id_bodega,id_medida_salida,lote,vencimiento, subtotal, ruc_empresa FROM cuerpo_proforma WHERE ruc_empresa = '" . $ruc_empresa . "' and codigo_unico='" . $codigo_unico . "'");
	...
	*/
			...
			select: function(event, ui) {
				...
				if (tipo_producto == "01" && configuracion_inventario == 'SI') {
					...
					$.post('../ajax/saldo_producto_inventario.php', {
						id_bode: bodega,
						id_prod: producto,
						editar_factura: codigo_proforma
					}).done(function(respuesta_salidas) {
	...
	$(function() {
		$('#serie_proforma').change(function() {
			...
			$.post('../ajax/consulta_configuracion_proformas.php', {
	...
	function aplicar_descuento_item() {
		...
		var serie_proforma = $("#serie_proforma").val();
		var secuencial_proforma = $("#secuencial_proforma").val();
		var subtotal_sin_impuesto = $("#subtotal_inicial").val();

		$.ajax({
			type: "POST",
			url: "../ajax/agregar_facturacion.php",
			data: "id_tmp_descuento=" + id + "&valor_descuento=" + descuento_item + "&id_cliente=" + id_cliente + "&serie_factura_e=" + serie_proforma + "&secuencial_factura=" + secuencial_proforma + "&subtotal_sin_impuesto=" + subtotal_sin_impuesto,
			beforeSend: function(objeto) {
				$("#resultados").html("Cargando...");
			},
			success: function(datos) {
