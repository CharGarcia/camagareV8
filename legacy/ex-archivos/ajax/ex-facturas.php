...
function subtotales_factura($con, $ruc_empresa, $serie_factura, $id_usuario)
{
	...
	// Consulta detalle de cada línea
	...
	while ($row = mysqli_fetch_assoc($sql_detalle)) {
		// Calculamos base imponible de la línea
		$base = round($row['subtotal'] - $row['descuento'], 2);

		// Calculamos IVA de la línea
		$iva_linea = 0;
		if ($row['porcentaje_iva'] > 0) {
			$iva_linea = round($base * ($row['porcentaje_iva'] / 100), 2);
		}

		// Acumulamos totales
		$subtotal_general += $base;
		$total_descuento += round($row['descuento'], 2);
		$suma_iva += $iva_linea;

		// Acumulamos subtotales por tarifa
		$tarifa_nombre = strtoupper($row['nombre_tarifa']);
		if (!isset($subtotal_tarifas[$tarifa_nombre])) {
			$subtotal_tarifas[$tarifa_nombre] = 0;
			$iva_tarifas[$tarifa_nombre] = 0;
		}

		$subtotal_tarifas[$tarifa_nombre] += $base;
		$iva_tarifas[$tarifa_nombre] += $iva_linea;
	}

	// Redondeamos totales finales
	$subtotal_general = round($subtotal_general, 2);
	$total_descuento = round($total_descuento, 2);
	$suma_iva = round($suma_iva, 2);

	// Propina y tasa turística
	$propina = mysqli_query($con, "
		SELECT * FROM configuracion_facturacion 
		WHERE ruc_empresa = '$ruc_empresa'
		AND serie_sucursal = '$serie_factura'
	");
	$row_propina = mysqli_fetch_assoc($propina);
...
?>
	...
	<div class="table-responsive">
		<table class="table table-bordered" style="padding: 0px; margin-bottom: 0px;">
			...
			<?php
			// Mostrar subtotales por tarifa
			foreach ($subtotal_tarifas as $nombre_tarifa => $valor_tarifa) {
			?>
				<tr class="info">
					<td class="text-right" style="padding: 2px;">
						Subtotal <?php echo strtolower($nombre_tarifa); ?>:
					</td>
					<td class="text-right" style="padding: 2px;">
						<?php echo number_format($valor_tarifa, 2, '.', ''); ?>
					</td>
				</tr>
			<?php
			}
					...
					?>
						...
						<tr>
							...
							<td class='col-md-2'><span class="pull-right">
									...
									?>
													<a class='btn btn-success btn-xs' onclick="enviar_factura_sri('<?php echo $id_encabezado_factura; ?>');" title='Enviar al SRI' data-toggle="modal" data-target="#EnviarDocumentosSri"><i class="glyphicon glyphicon-send"></i></a>
												<?php
												}
												break;
										}
