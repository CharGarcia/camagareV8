...
if ($action == 'buscar_proveedores') {
	...
	$sTable = "proveedores as pro LEFT JOIN encabezado_compra as enc ON enc.id_proveedor=pro.id_proveedor";
	$sWhere = "WHERE pro.ruc_empresa = '" . $ruc_empresa . "' ";

	// ====== FILTRO DE BUSQUEDA POR PALABRAS (AND por cada palabra) ======
	if (!empty($q)) {
		$words = preg_split('/\s+/', trim($q));
		foreach ($words as $w) {
			$w = mysqli_real_escape_string($con, $w);
			$ors = array();
			foreach ($aColumns as $col) {
				$ors[] = "pro.$col LIKE '%$w%'";
			}
			$sWhere .= " AND (" . implode(" OR ", $ors) . ") ";
		}
	}
	// ====== ORDEN ======
	$sOrder = " ORDER BY pro.razon_social ASC ";
...
";
	...
	// ====== QUERY PRINCIPAL (todos los proveedores + conteo de compras) ======
	$sql = "
SELECT
  pro.*,
  COUNT(enc.id_proveedor) AS total_compras
FROM $sTable
$sWhere
GROUP BY pro.id_proveedor
$sOrder
				...
				<table class="table">
					...
					?>
						<tr>
							...
							<td title="Compras realizadas"><span class="label label-info"><span class="glyphicon glyphicon-shopping-cart"></span> <?php echo (int)$row['total_compras']; ?></span></td>
							...
							<td><span class="pull-right">
									...
									?>
										<a class='btn btn-info btn-xs' title='Editar proveedor' onclick="editar_proveedor('<?php echo $id_proveedor; ?>');" data-toggle="modal" data-target="#modalProveedor"><i class="glyphicon glyphicon-edit"></i></a>
									<?php
									}
									if (getPermisos($con, $id_usuario, $ruc_empresa, 'proveedores')['d'] == 1) {
									?>
										<a class='btn btn-danger btn-xs' title='Eliminar proveedor' onclick="eliminar_proveedor('<?php echo $id_proveedor; ?>');"><i class="glyphicon glyphicon-trash"></i></a>
									<?php
									}
									?>
						</tr>
