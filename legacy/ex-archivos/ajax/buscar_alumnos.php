...
if ($action == 'buscar_alumnos') {
	...
	if (!empty($q)) {
		...
		// Puedes concatenar las columnas para una búsqueda más completa
		$camposBusqueda = [
			"CONCAT_WS(' ', al.nombres_apellidos, cl.nombre, cl.ruc, cl.email)",
			"CONCAT_WS(' ', cl.nombre, al.nombres_apellidos)",
			"CONCAT_WS(' ', cl.email, al.nombres_apellidos)"
		];
		...
		foreach ($camposBusqueda as $campo) {
			$subcond = [];
			foreach ($text_buscar as $palabra) {
				$palabra = trim($palabra);
				if ($palabra !== "") {
					$subcond[] = "$campo LIKE '%$palabra%'";
				}
			}
			if (!empty($subcond)) {
				$condiciones[] = '(' . implode(' AND ', $subcond) . ')';
			}
		}

		if (!empty($condiciones)) {
			$sWhere .= implode(' OR ', $condiciones);
		} else {
			$sWhere .= "1"; // para evitar error de sintaxis si no hay palabras válidas
		}
