			...
			<div class="panel panel-warning">
				<table class="table table-hover">
					...
					while ($row = mysqli_fetch_array($query)) {
						...
						$id_encrypt = encrypt_decrypt("encrypt", $id_consignacion);
					...
					?>
						<tr>
							...
							<td class='text-right'>
								<a title="Imprimir pdf" href="../ajax/imprime_documento.php?action=pdf_retorno_consignacion_venta&path=<?php echo $path; ?>&tipoDescarga=<?php echo $tipoDescarga; ?>&id_consignacion=<?php echo $id_encrypt; ?>" class='btn btn-default btn-xs' title='Pdf' target="_blank">Pdf</a>
								<a class='btn btn-info btn-xs' title='Detalle devolución' onclick="mostrar_detalle_devolucion_consignacion('<?php echo $codigo_unico; ?>');" data-toggle="modal" data-target="#detalleConsignacion"><i class="glyphicon glyphicon-list"></i></a>
								<a class='btn btn-danger btn-xs' title='Eliminar consignación' onclick="eliminar_devolucion_consignacion_ventas('<?php echo $codigo_unico; ?>','<?php echo $numero; ?>');"><i class="glyphicon glyphicon-trash"></i></a>
								<?php
								if (getPermisos($con, $id_usuario, $ruc_empresa, 'devolucion_consignacion_venta')['r'] == 1) {
								?>
									<a class="btn btn-info btn-xs" onclick="enviar_retornos_mail('<?php echo $id_consignacion; ?>', '<?php echo $mail; ?>');" title='Enviar por mail' data-toggle="modal" data-target="#EnviarDocumentosMail"><i class="glyphicon glyphicon-envelope"></i> </a>
								<?php
								}
								?>
							</td>
