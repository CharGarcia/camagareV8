...
if ($action === 'detalle_pedido') {
	detalle_pedido($con, $_SESSION['ruc_empresa'], $_SESSION['id_usuario']);
	exit;
}


function detalle_pedido($con, $ruc_empresa, $id_usuario)
{
	// Lee y valida inputs
	$pedido = isset($_GET['pedido']) ? (int)$_GET['pedido'] : 0;
	$bodega = isset($_GET['bodega']) ? (int)$_GET['bodega'] : 0;

	if ($pedido <= 0) {
		echo "<div class='alert alert-warning'>Pedido inválido.</div>";
		return;
	}

	// 1) Traer renglones del pedido (1 sola consulta)
	$sqlPedido = "
        SELECT 
            det.id                          AS id_detalle,
            enc.numero_pedido               AS numero_pedido,
            det.id_producto                 AS id_producto,
            det.codigo_producto             AS codigo_producto,
            det.producto                    AS producto,
            det.cantidad                    AS cantidad,
            ...
            cli.nombre                      AS cliente,
            cli.id_vendedor                 AS id_vendedor,
            enc.observaciones_cliente       AS observaciones_cliente,
            enc.hora_entrega_desde          AS hora_entrega_desde,
            enc.hora_entrega_hasta          AS hora_entrega_hasta,
            enc.fecha_entrega               AS fecha_entrega,
            enc.responsable                 AS responsable,
            pro.precio_producto             AS precio
        FROM detalle_pedido det
        INNER JOIN encabezado_pedido enc ON enc.id = det.id_pedido
        INNER JOIN productos_servicios pro ON pro.id = det.id_producto
        INNER JOIN clientes cli ON cli.id = enc.id_cliente
        WHERE enc.numero_pedido = ?
          AND enc.ruc_empresa  = ?
          AND enc.status      != 3
        ORDER BY det.id ASC
    ";
	$items = [];
	if ($st = mysqli_prepare($con, $sqlPedido)) {
		mysqli_stmt_bind_param($st, 'is', $pedido, $ruc_empresa);
		...
		echo "<div class='alert alert-danger'>No se pudo cargar el pedido.</div>";
		return;
	}

	if (empty($items)) {
		echo "<div class='alert alert-info'>No hay detalles para este pedido.</div>";
		return;
	}

	// Armar lista de productos únicos
	$prodIds = [];
	foreach ($items as $it) {
		$prodIds[(int)$it['id_producto']] = true;
	}
	$prodIds = array_keys($prodIds);
	$inPlaceholders = implode(',', array_fill(0, count($prodIds), '?'));

	// 2) “Agregado” en factura_tmp para TODOS los productos (1 consulta)
	$agregadoMap = []; // id_producto => sum(cantidad_tmp)
	if (!empty($prodIds)) {
		$sqlAgg = "
            SELECT id_producto, SUM(cantidad_tmp) AS agregado
              FROM factura_tmp
             WHERE id_usuario = ?
               AND ruc_empresa = ?
               AND id_producto IN ($inPlaceholders)
             GROUP BY id_producto
        ";
		if ($st = mysqli_prepare($con, $sqlAgg)) {
			// Bind dinámico: 's' o 'i' según tu esquema (id_usuario), aquí se usa string para compatibilidad
			$types = 'ss' . str_repeat('i', count($prodIds));
			$params = array_merge([$types, $id_usuario, $ruc_empresa], $prodIds);
			// trucazo para bind_param dinámico
			$refs = [];
			...
			$rs = mysqli_stmt_get_result($st);
			while ($row = mysqli_fetch_assoc($rs)) {
				$agregadoMap[(int)$row['id_producto']] = (float)$row['agregado'];
			}
			mysqli_stmt_close($st);
		}
	}

	// 3) Precio normal por producto (1 consulta)
	$precioNormal = []; // id_producto => precio_producto
	if (!empty($prodIds)) {
		$sqlPrecioBase = "SELECT id, precio_producto FROM productos_servicios WHERE id IN ($inPlaceholders)";
		if ($st = mysqli_prepare($con, $sqlPrecioBase)) {
			$types = str_repeat('i', count($prodIds));
			$params = array_merge([$types], $prodIds);
			$refs = [];
			foreach ($params as $k => $v) $refs[$k] = &$params[$k];
			call_user_func_array([$st, 'bind_param'], $refs);
			...
			$rs = mysqli_stmt_get_result($st);
			while ($row = mysqli_fetch_assoc($rs)) {
				$precioNormal[(int)$row['id']] = (float)$row['precio_producto'];
			}
			mysqli_stmt_close($st);
		}
	}

	// 4) Listas de precios vigentes para TODOS los productos (1 consulta)
	$fecha_actual = date('Y-m-d H:i:s');
	$preciosVigentes = []; // id_producto => [ [detalle_precio, precio], ... ]
	if (!empty($prodIds)) {
		$sqlListas = "
            SELECT id_producto, detalle_precio, precio
              FROM precios_productos
             WHERE id_producto IN ($inPlaceholders)
               AND DATE_FORMAT(?, '%Y/%m/%d') BETWEEN DATE_FORMAT(fecha_desde, '%Y/%m/%d')
                                                     AND DATE_FORMAT(fecha_hasta, '%Y/%m/%d')
             ORDER BY detalle_precio ASC
        ";
		if ($st = mysqli_prepare($con, $sqlListas)) {
			$types = str_repeat('i', count($prodIds)) . 's';
			// ojo: el ? de la fecha va al final por cómo está escrito el SQL
			$params = array_merge([$types], $prodIds, [$fecha_actual]);
			$refs = [];
			foreach ($params as $k => $v) $refs[$k] = &$params[$k];
			call_user_func_array([$st, 'bind_param'], $refs);
			...
			while ($row = mysqli_fetch_assoc($rs)) {
				...
				if (!isset($preciosVigentes[$pid])) $preciosVigentes[$pid] = [];
				$preciosVigentes[$pid][] = [
	...
	<div class="panel panel-info">
		<div class="table-responsive">
			<table class="table table-hover">
				...
				<?php foreach ($items as $detalle):
					$id_detalle      = (int)$detalle['id_detalle'];
					$id_cliente      = (int)$detalle['id_cliente'];
					$cliente         = $detalle['cliente'];
					$numero_pedido   = (int)$detalle['numero_pedido'];
					$id_producto     = (int)$detalle['id_producto'];
					$codigo_producto = $detalle['codigo_producto'];
					$nombre_producto = $detalle['producto'];
					$cantidad        = (float)$detalle['cantidad'];
					$despachado      = (float)$detalle['despachado'];
					$medida          = (int)$detalle['medida'];
					$observaciones   = $detalle['observaciones_cliente'];
					$hora_desde      = $detalle['hora_entrega_desde'] ? date('H:i', strtotime($detalle['hora_entrega_desde'])) : '';
					$hora_hasta      = $detalle['hora_entrega_hasta'] ? date('H:i', strtotime($detalle['hora_entrega_hasta'])) : '';
					$fecha_entrega   = $detalle['fecha_entrega'] ? date('d-m-Y', strtotime($detalle['fecha_entrega'])) : '';
					$responsable     = $detalle['responsable'];
					// Precio base (preferimos el actual de productos_servicios)
					$precio_base     = isset($precioNormal[$id_producto]) ? $precioNormal[$id_producto] : (float)$detalle['precio'];
					// Agregado actual del usuario
					$agregado_val    = isset($agregadoMap[$id_producto]) ? $agregadoMap[$id_producto] : 0.0;

					$saldo_pendiente = max(0, $cantidad - $despachado);
					$agregado_html   = ($agregado_val > $saldo_pendiente)
						? '<span class="label label-danger">' . number_format($agregado_val, 0, '.', '') . '</span>'
						: number_format($agregado_val, 0, '.', '');

					// Precios vigentes para este producto
					$lista_precios = isset($preciosVigentes[$id_producto]) ? $preciosVigentes[$id_producto] : [];
				?>
					<!-- Hidden fields conservando tus IDs para el front -->
					<input type="hidden" id="hora_entrega_desde<?php echo $id_detalle; ?>" value="<?php echo htmlspecialchars($hora_desde, ENT_QUOTES); ?>">
					<input type="hidden" id="hora_entrega_hasta<?php echo $id_detalle; ?>" value="<?php echo htmlspecialchars($hora_hasta, ENT_QUOTES); ?>">
					<input type="hidden" id="fecha_entrega<?php echo $id_detalle; ?>" value="<?php echo htmlspecialchars($fecha_entrega, ENT_QUOTES); ?>">
					<input type="hidden" id="responsable<?php echo $id_detalle; ?>" value="<?php echo htmlspecialchars($responsable, ENT_QUOTES); ?>">
					<input type="hidden" id="observaciones_pedido<?php echo $id_detalle; ?>" value="<?php echo htmlspecialchars($observaciones, ENT_QUOTES); ?>">
					<input type="hidden" id="numero_pedido<?php echo $id_detalle; ?>" value="<?php echo $numero_pedido; ?>">
					<input type="hidden" id="id_producto_pedido<?php echo $id_detalle; ?>" value="<?php echo $id_producto; ?>">
					<input type="hidden" id="id_medida_pedido<?php echo $id_detalle; ?>" value="<?php echo $medida; ?>">
					<input type="hidden" id="id_cliente_pedido<?php echo $id_detalle; ?>" value="<?php echo $id_cliente; ?>">
					<input type="hidden" id="nombre_cliente_pedido<?php echo $id_detalle; ?>" value="<?php echo htmlspecialchars($cliente, ENT_QUOTES); ?>">
					<input type="hidden" id="id_vendedor<?php echo $id_detalle; ?>" value="<?php echo (int)$detalle['id_vendedor']; ?>">
					<input type="hidden" id="saldo_entrante<?php echo $id_detalle; ?>" value="<?php echo number_format($saldo_pendiente, 2, '.', ''); ?>">

					<tr>
						<td style="padding:2px;"><?php echo htmlspecialchars($codigo_producto, ENT_QUOTES); ?></td>
						<td style="padding:2px;" class="col-xs-2"><?php echo htmlspecialchars($nombre_producto, ENT_QUOTES); ?></td>
						<td style="padding:2px;"><?php echo number_format($saldo_pendiente, 0, '.', ''); ?></td>
						<td style="padding:2px;"><?php echo $agregado_html; ?></td>

						<td style="padding:2px;">
							<select class="form-control input-sm" title="Seleccione precio" style="width:120px;" id="lista_precios_pedido<?php echo $id_detalle; ?>" onChange="lista_precios_pedido(<?php echo $id_detalle; ?>)">
								...
								<?php foreach ($lista_precios as $lp): ?>
									<option value="<?php echo number_format($lp['precio'], 2, '.', ''); ?>">
										<?php echo htmlspecialchars($lp['detalle_precio'], ENT_QUOTES) . ' ' . number_format($lp['precio'], 2, '.', ''); ?>
									</option>
								<?php endforeach; ?>
							</select>
						</td>
						...
						<td style="padding:2px;" class="col-xs-1">
							<input type="text" class="form-control input-sm" style="text-align:right;" title="Ingrese precio"
								name="precio_pedido" id="precio_pedido<?php echo $id_detalle; ?>" placeholder="Precio"
								value="<?php echo number_format($precio_base, 2, '.', ''); ?>">
						</td>
						...
						<td style="padding:2px;">
							<select class="form-control input-sm"
								...
								name="lote_pedido"
								id="lote_pedido<?php echo $id_detalle; ?>"
								onChange="saldo_producto_pedido('<?php echo $id_detalle; ?>');">
								<option value="0" selected>Seleccione</option>
							</select>
						</td>

						<td style="padding:2px;">
							<input type="text" class="form-control input-sm" style="text-align:right; width:50px;" title="Ingrese cantidad"
								name="cantidad_pedido" id="cantidad_pedido<?php echo $id_detalle; ?>" placeholder="Cant" value="1">
						</td>

						<td style="padding:2px;" class="col-xs-1">
							<div class="pull-right">
								<input type="text" class="form-control input-sm" style="text-align:right;" title="Ingrese número único de producto"
									name="nup_pedido" id="nup_pedido<?php echo $id_detalle; ?>" placeholder="nup">
							</div>
						</td>

						<td style="padding:2px;" class="col-xs-1">
							<input type="text" style="text-align:right;" class="form-control input-sm"
								name="existencia_pedido" id="existencia_pedido<?php echo $id_detalle; ?>" readonly>
						</td>

						<td style="padding:2px;" class="text-right">
							<a href="#" class="btn btn-info btn-xs" title="Agregar" onclick="agregar_item_pedido('<?php echo $id_detalle; ?>')">
...
// ⚠️ Este es un posible cuello de botella. Si lotes() es costoso, ver “Tips performance” abajo.
/* $lotes_encontrados = lotes($id_producto, $bodega, $con, $ruc_empresa);
								if (is_array($lotes_encontrados)) {
									foreach ($lotes_encontrados as $lotes) {
										$lote = isset($lotes['lote']) ? $lotes['lote'] : '';
										$fcad = isset($lotes['fecha_caducidad']) ? $lotes['fecha_caducidad'] : null;
										$mY   = $fcad ? date('m-Y', strtotime($fcad)) : '';
										echo '<option value="' . htmlspecialchars($lote, ENT_QUOTES) . '">' . htmlspecialchars($lote, ENT_QUOTES) . ' vence:' . $mY . '</option>';
									}
								} */
...
if ($action == 'obtener_lotes') {
	...
	if ($id_producto > 0 && $bodega > 0 && $ruc_empresa) {
		$lotes = lotes($id_producto, $bodega, $con, $ruc_empresa);
...
function lotes($id_producto, $bodega, $con, $ruc_empresa)
{
	// Normaliza tipos
	$id_producto = (int)$id_producto;
	$bodega      = (int)$bodega;
	$ruc_empresa = (string)$ruc_empresa;

	// 1) Agrega por lote y (opcional) usa fecha de vencimiento de entradas
	$sql = "
        SELECT 
            i.lote,
            SUM(i.cantidad_entrada) AS entradas,
            SUM(i.cantidad_salida)  AS salidas,
            MAX(CASE WHEN i.operacion='ENTRADA' THEN i.fecha_vencimiento ELSE NULL END) AS fecha_venc
        FROM inventarios i
        WHERE i.ruc_empresa = ?
          AND i.id_producto = ?
          AND i.id_bodega   = ?
        GROUP BY i.lote
        HAVING (SUM(i.cantidad_entrada) - SUM(i.cantidad_salida)) > 0
        ORDER BY fecha_venc ASC
    ";

	$out = [];
	if ($st = mysqli_prepare($con, $sql)) {
		mysqli_stmt_bind_param($st, 'sii', $ruc_empresa, $id_producto, $bodega);
		mysqli_stmt_execute($st);
		$rs = mysqli_stmt_get_result($st);
		while ($r = mysqli_fetch_assoc($rs)) {
			$saldo = (float)$r['entradas'] - (float)$r['salidas'];
			if ($saldo > 0) {
				$out[] = [
					'lote'            => (string)$r['lote'],
					'fecha_caducidad' => $r['fecha_venc'], // puede ser null si no hubo ENTRADA con fecha
					//'saldo'        => $saldo, // descomenta si quieres devolverlo
				];
			}
		}
		mysqli_stmt_close($st);
