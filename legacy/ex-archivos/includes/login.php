<?php
include_once("../sistema/conexiones/conectalogin.php");
...
{
?>
	<form id="formUnaEmpresa" method="POST" action="/sistema/empresa">
			...
			<div class="panel panel-info">
				<div class="panel-heading">
					<div class="panel-title">
						<h4><a href="https://www.camagare.com">CaMaGaRe.com</a></h4>
					</div>
				</div>
			</div>
 ...
 * Si la contraseña está en MD5, la migra automáticamente a bcrypt al loguearse.
 * @param string $cedula
 * @param string $passwordPlain Contraseña en texto plano (sin hashear)
...
function valida_login($cedula, $passwordPlain)
{
	...
	$stmt = mysqli_prepare($conexion, "SELECT id, nombre, cedula, password, nivel FROM usuarios WHERE cedula = ? AND estado = '1'");
	if (!$stmt) return false;

	mysqli_stmt_bind_param($stmt, "s", $cedula);
	mysqli_stmt_execute($stmt);

	// mysqli_stmt_get_result requiere mysqlnd; bind_result es compatible con más instalaciones
	$user = null;
	if (function_exists('mysqli_stmt_get_result')) {
		$result = mysqli_stmt_get_result($stmt);
		$user = $result ? mysqli_fetch_assoc($result) : null;
	} else {
		$id = $nombre = $ced = $pass = $nivel = null;
		mysqli_stmt_bind_result($stmt, $id, $nombre, $ced, $pass, $nivel);
		if (mysqli_stmt_fetch($stmt)) {
			$user = array('id' => $id, 'nombre' => $nombre, 'cedula' => $ced, 'password' => $pass, 'nivel' => $nivel);
		}
	}
	...
	// Contraseña en bcrypt (ya migrada)
	if (strpos($storedHash, '$2y$') === 0 || strpos($storedHash, '$2a$') === 0) {
		return password_verify($passwordPlain, $storedHash) ? $user : false;
	}

	// Contraseña en MD5 (legacy): validar y migrar a bcrypt
	if (strlen($storedHash) === 32 && ctype_xdigit($storedHash)) {
		if (md5($passwordPlain) === $storedHash) {
			$newHash = password_hash($passwordPlain, PASSWORD_DEFAULT);
			$userId = (int) $user['id'];
			$upd = mysqli_prepare($conexion, "UPDATE usuarios SET password = ? WHERE id = ?");
			if ($upd) {
