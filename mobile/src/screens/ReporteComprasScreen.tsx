...
import { obtenerReporteCompras, ReporteComprasProveedor, ReporteMes } from '../api/reportes';
...
export default function ReporteComprasScreen() {
  const hoy = new Date();
  ...
  const [porProveedor, setPorProveedor] = useState<ReporteComprasProveedor[]>([]);
  ...
  const cargar = useCallback(
    async (esRefresh = false) => {
      ...
      setError(null);
      try {
        const data = await obtenerReporteCompras({ fecha_desde: aIso(desde), fecha_hasta: aIso(hasta) });
        setEstadisticas(data.estadisticas);
        setPorMes(data.por_mes);
        setPorProveedor(data.por_proveedor);
      } catch (err) {
    ...
    >
      ...
      {cargando ? (
        ...
        <>
          ...
          <View style={styles.card}>
            <Text style={styles.seccionTitulo}>Por proveedor</Text>
            {porProveedor.length === 0 ? (
              <Text style={styles.vacio}>Sin datos en el rango seleccionado.</Text>
            ) : (
              porProveedor.map((p) => (
                <View key={p.id_proveedor} style={styles.filaLista}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.itemNombre} numberOfLines={1}>
                      {p.proveedor_nombre}
                    </Text>
                    <Text style={styles.itemSub}>{p.proveedor_ruc}</Text>
                  </View>
                  <Text style={styles.itemTotal}>${Number(p.total).toFixed(2)}</Text>
                </View>
              ))
            )}
          </View>
