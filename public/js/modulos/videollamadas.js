...
(function () {
    ...
    window.VC_borrarSecreto = async function (campo) {
        if (typeof Swal !== 'undefined') {
            const r = await Swal.fire({
                ...
                cancelButtonText: 'Cancelar',
                confirmButtonColor: '#dc3545',
                target: document.getElementById('modalConfigVC'),
            });
            if (!r.isConfirmed) return;
        }
    ...
    window.VC_probarTurn = async function () {
        ...
        caja.innerHTML = '<div class="text-muted small"><span class="spinner-border spinner-border-sm me-1"></span>Consultando...</div>';

        try {
            const res = await (await fetch(`${VC_URL}/probarTurnAjax`)).json();
            if (!res.ok) {
                caja.innerHTML = `<div class="alert alert-danger py-2 px-3 mb-0 small">${esc(res.mensaje)}</div>`;
                return;
            }
            const tipo = res.turn > 0 ? 'success' : 'warning';
            caja.innerHTML =
                `<div class="alert alert-${tipo} py-2 px-3 mb-0 small">` +
                `<strong>${res.stun}</strong> servidor(es) STUN y <strong>${res.turn}</strong> TURN disponibles.` +
                (res.aviso ? '<br>' + esc(res.aviso) : '') +
                '</div>';
        } catch (e) {
            caja.innerHTML = '<div class="alert alert-danger py-2 px-3 mb-0 small">No se pudo consultar.</div>';
        }
    };
