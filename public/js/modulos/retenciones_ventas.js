(function () {
    ...
    // la retención recibida y —con permiso de actualizar en Asientos Contables— permite
    // corregirlo y guardarlo, validando que siga cuadrando con el total retenido.
    let _retvAsientoTab = null;
    function retvAsientoTab() {
        // Sin permiso sobre Asientos Contables la pestaña no se renderiza: nada que inicializar.
        if (!document.getElementById('retv-asiento-tbody')) return null;
        if (!_retvAsientoTab && typeof window.crearAsientoTab === 'function') {
            _retvAsientoTab = window.crearAsientoTab({
                prefijo: 'retv',
                moduloOrigen: 'retencion_venta',
        ...
        return _retvAsientoTab;
    }

    function cargarAsientoContable(id) {
        const tab = retvAsientoTab();
