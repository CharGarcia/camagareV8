...
**Tarjeta de control fija (reportes/listados con filtros + KPIs)**
- En páginas que ya aplican la excepción anterior (app-shell desactivado, filtros/KPI encima de la tabla), el título del módulo, los filtros y las tarjetas pequeñas de KPI **van dentro de una única tarjeta**, no sueltos como bloques separados.
- Esa tarjeta **no es colapsable/escondible**: siempre está totalmente visible y queda **fija (`position: sticky`) pegada justo debajo del navbar** al hacer scroll de la página.
- **Clase**: `cmg-control-card` (estilos globales ya definidos en `public/css/app.css`, sección "Tarjeta de control fija"). Usa `top: var(--cmg-sticky-h)`, la misma variable que ya mantiene actualizada `public/js/partials` con la altura real del header — no hay que calcular nada a mano.
- **Estructura interna — head / body / footer, como una `.card` estándar de Bootstrap**:
  - **`card-header`** (`bg-white border-bottom py-2 px-3`): solo el `<h5>` con **ícono + nombre del módulo** (p. ej. `<i class="bi bi-wallet2 me-2 text-success"></i>Cuentas por Cobrar`).
  - **`card-body`** (`p-3`): únicamente el `<form>` de filtros/selectores y sus botones (Limpiar / Aplicar) — sin acordeón, sin KPIs.
    - **El `<form>` es flexbox puro (`d-flex flex-wrap align-items-start gap-2`), NO el grid `.row`/`.col-*` de Bootstrap.** Cada campo es un `<div>` sin clase `col-*`, con ancho fijo por `style="width:…px"` (o `position-relative` + ancho fijo para el buscador con dropdown). Se probó con `.row` mezclando columnas `col-md-auto` (ancho por contenido) y una `col-md-3` (ancho porcentual, para el buscador) y el resultado fue inconsistente: según el ancho disponible el grid partía la fila en puntos distintos, dejando el buscador o los botones saltando a su propia línea o desplazados — con `flex-wrap` simple y anchos fijos en cada campo, el punto de quiebre es predecible y todos los campos se comportan igual.
    - **El buscador (cliente/proveedor/etc.) y los botones (Limpiar/Aplicar) van agrupados dentro de un `<div class="d-flex flex-wrap align-items-start gap-2">` propio**, anidado como un solo ítem dentro del `<form>`. Así, si la fila no tiene espacio para todos los campos, ese par salta junto a la siguiente línea — nunca se separan entre sí (el buscador nunca queda en una línea y los botones en otra).
    - `align-items-start` (no `align-items-end`): cada campo empieza con su `<label>` (mismo estilo/alto en todos), así que las etiquetas — y por tanto los controles justo debajo — quedan siempre alineadas por arriba, sin depender de que cada control (select, input, `input-group` con ícono, botones) tenga exactamente el mismo alto renderizado. `align-items-end` se probó y se descartó: un campo con un `input-group` (p. ej. el buscador con ícono) puede renderizar unos px más alto/bajo que un `.form-select-sm` plano, y al alinear por abajo eso empuja el resto de campos hacia abajo dejándolos visualmente "más bajos" que ese campo — con `align-items-start` ese desajuste de alturas no se nota porque la alineación no depende de él.
    - Todos los `<label>` del formulario, **incluida la etiqueta invisible de la columna de botones, llevan `d-block`** (no confiar en que `.form-label` sea block por defecto: `<label>` es `inline` salvo que se fuerce, y en un elemento inline el `margin-bottom` no genera espacio real — si solo el label invisible tiene `d-block` y los demás no, ese campo gana un espaciado real que los otros no tienen, y eso desalinea la fila).
    - La columna de botones usa un `<label>&nbsp;</label>` invisible (con `d-block`, igual que el resto) para igualar la altura de la etiqueta y que los botones arranquen a la misma altura que los inputs. Si un campo tiene un contenedor auxiliar que puede quedar vacío (p. ej. chips de selección múltiple), neutralizar igualmente su margen con `#id-contenedor:empty { margin-top:0; }`.
    - **Altura de los controles, explícita y forzada — no confiar en que `-sm` rinda igual entre tipos de control.** `.form-select-sm`, `.form-control-sm`, `.input-group-sm` y `.btn-sm` de Bootstrap no siempre calculan el mismo alto en píxeles (el buscador con ícono dentro de un `.input-group` es el caso más propenso a diferir), y eso rompe la alineación incluso con `align-items-start`. Forzar el mismo alto por CSS sobre el `<form>` del módulo: `#form-filtros-{modulo} .form-select, .form-control, .input-group-text, .btn { height:28px; font-size:.75rem; }` — controles compactos, acorde al resto de la tarjeta.
    - Anchos de referencia (compactos): selects ~130-140px, fechas ~115px, buscador (cliente/proveedor/etc.) ~440px (más ancho que el resto para que el nombre no se corte).
  - **`card-footer`** (`bg-white border-top py-2 px-3`): un `<div class="cmg-control-card__stats">` con el resumen de documentos y demás indicadores en formato compacto: cada uno es un `<div class="cmg-control-card__stat">` con un ícono (`bg-{color} bg-opacity-10 text-{color}`) y, al lado, el valor (`cmg-control-card__stat-value`) y la etiqueta (`cmg-control-card__stat-label`) — p. ej. ícono de documentos + número de documentos, ícono de billetera + saldo total, etc. El borde/fondo del footer ya separa visualmente del body; no agregar otro borde dentro de `cmg-control-card__stats`.
- **La tabla debajo NO lleva scroll interno acotado**: a diferencia de la regla general de la excepción anterior (`.{modulo}-scroll { max-height: …; overflow: auto; }`), en páginas con `cmg-control-card` el listado se deja **libre para extenderse hacia abajo** — quien scrollea es la página completa, no un contenedor interno. La tarjeta de control ya resuelve "qué queda visible" quedándose fija arriba; el `.{modulo}-scroll` solo necesita `overflow-x: auto` (para el scroll horizontal si la tabla no cabe) y su `thead th` puede mantener `background`/`box-shadow` pero **sin `position: sticky`** (ya no hay un contenedor interno al cual pegarse). En el media query móvil (`max-width: 767.98px`), `app.css` fuerza `max-height` en cualquier `[class*="-scroll"]` con `!important`; hay que neutralizarlo explícitamente por módulo con un selector de mayor especificidad, p. ej. `#modulo-{ruta} .{modulo}-scroll { max-height:none !important; height:auto !important; overflow-y:visible !important; }`.
- **Referencia canónica**: `app/views/modulos/cuentas_por_cobrar/index.php`.
- No confundir con la tarjeta de listado (`cmg-table-card`, ver regla anterior): la tarjeta de control es la de **arriba** (filtros/KPI), la de listado es la de la **tabla**.
...
- **Nada de pestaña "Información"**: en los módulos nuevos NO se agrega una pestaña
  *Información* (tarjeta de permisos + datos de auditoría) al modal. Los permisos ya se
  administran en `/config/permisos-modulos` y el historial del registro se consulta en
...
- **PDF de un documento desde un modal (obligatorio)**: el botón PDF (o Imprimir) de **todo** modal, existente o futuro, llama a **`CMG_pdfDocumento(url)`** (`public/js/app.js`), que genera el PDF con el aviso "Generando PDF…" y luego pregunta **Imprimir / Descargar / Ver**. Imprimir abre el cuadro de impresión del navegador con el PDF ya cargado (iframe oculto); en celulares abre el PDF en otra pestaña. **Nunca** usar para esto `window.open(...)`, un `<a>` temporal con `click()` ni `CMG_descargar()`. Imprimir sin el cuadro de impresión es imposible desde una página web; no intentarlo.
  ...
  - **Enlaces armados como HTML** (celdas de tablas dentro de modales, pestañas de fichas, filas que devuelve el servidor): basta el atributo **`data-pdf-documento`** en el `<a href="…">` (sin `target="_blank"` ni `download`); un listener global de `app.js` lo intercepta y llama a `CMG_pdfDocumento`. Funciona aunque el enlace tenga `onclick="event.stopPropagation()"`.
