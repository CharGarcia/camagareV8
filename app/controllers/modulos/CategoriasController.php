...
use App\Services\modulos\CategoriaService;

class CategoriasController extends BaseModuloController
{
    private const RUTA_MODULO = 'modulos/categorias';
    ...
    public function __construct()
    {
        ...
        $rules = new CategoriaRules();
        $logService = new LogSistemaService();
        $this->service = new CategoriaService($repo, $rules, $logService);
    }
    ...
    public function index(): void
    {
        ...
        // Formateo para las fechas de auditoría que se precargarán en el modal
        foreach ($rows as &$r) {
        ...
        $this->viewWithLayout('layouts.main', 'modulos.categorias.index', [
            'titulo'     => 'Categorías',
            'perm'       => $perm,
    ...
    public function searchAjax(): void
    {
        ...
        ob_start();
        if (empty($rows)) {
            echo '<tr><td colspan="3" class="text-center py-5 text-muted"><i class="bi bi-tags fs-3 d-block mb-2"></i>No se encontraron categorías.</td></tr>';
        } else {
            foreach ($rows as $r) {
                if (!empty($r['created_at'])) $r['created_at'] = date('d-m-Y H:i:s', strtotime($r['created_at']));
                if (!empty($r['updated_at'])) $r['updated_at'] = date('d-m-Y H:i:s', strtotime($r['updated_at']));
                ...
                $count = $r['productos_count'] ?? 0;
                echo '<tr class="categoria-row" role="button" tabindex="0" data-row=\'' . $dataAttr . '\' onclick="abrirModalCategoriaEditar(this)">
                        <td class="ps-3 fw-bold" data-col="nombre">' . htmlspecialchars($r['nombre'] ?? '') . '</td>
                        <td class="text-center" data-col="status">' . $statusBadge . '</td>
                        <td class="text-end pe-3 small text-muted">' . $count . ' art.</td>
                      </tr>';
            }
        }
    ...
    public function getDetalleAjax(): void
    {
        ...
        try {
            ...
            $repo = new CategoriaRepository();
            $categoria = $repo->getDetalleCompleto($id, $idEmpresa);
            ...
            echo json_encode([
                'ok' => true,
                'data' => [
                    'creado_at' => $fmt($categoria['created_at'] ?? $categoria['creado_at'] ?? null),
                    'creado_por' => $categoria['creado_por_nombre'] ?? 'Sistema',
                    'actualizado_at' => $fmt($categoria['updated_at'] ?? $categoria['actualizado_at'] ?? null),
                    'actualizado_por' => $categoria['actualizado_por_nombre'] ?? '—',
    ...
    public function store(): void
    {
        ...
        try {
            ...
            echo json_encode(['ok' => true, 'msg' => 'Categoría creada correctamente.', 'id' => $id]);
        } catch (\Throwable $e) {
            echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
        }
        exit;
    ...
    public function update(): void
    {
        ...
        try {
            if ($id <= 0) throw new \Exception('ID de la categoría no es válido.');
            $this->service->actualizar($id, $idEmpresa, $data);
            echo json_encode(['ok' => true, 'msg' => 'Categoría actualizada correctamente.']);
        } catch (\Throwable $e) {
            echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
        }
        exit;
    ...
    public function exportPdf(): void
    {
        ...
        try {
            ...
            $nombreEmpresa = $empresa['nombre'] ?? 'REPORTE DE CATEGORÍAS';
...
<?php
            ...
            $html2pdf->output('Categorias_' . date('Ymd_His') . '.pdf', 'D');
            exit;
        } catch (\Throwable $e) {
    ...
    public function exportExcel(): void
    {
        ...
        try {
            ...
            $reportService->exportToExcel('Categorías', $headers, $exportData, 'Listado Categorías', $nombreEmpresa);
            exit;
        } catch (\Throwable $e) {
            header('Content-Type: text/html');
