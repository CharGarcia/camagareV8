 ...
 * Servicio Externo.
 *
 * Registra órdenes de mantenimiento realizado en el sitio del cliente, con sus
 * repuestos (de bodega) y mano de obra/servicio (libre). Desde la orden se puede
 * generar un documento de venta (Factura SRI o Recibo de Venta).
 */
class ServicioExternoController extends BaseModuloController
{
    private ServicioExternoService $service;
    private ServicioExternoRepository $repository;
    private const RUTA_MODULO = 'modulos/servicio-externo';
    ...
    public function __construct()
    {
        ...
        $rules      = new ServicioExternoRules();
        $logService = new LogSistemaService();
        $this->service = new ServicioExternoService($this->repository, $rules, $logService);
    }
    ...
    public function index(): void
    {
        ...
        $ordenCol = trim($_GET['sort'] ?? $prefsVista['__ordenCol__'] ?? 'fecha_servicio');
        ...
        $this->viewWithLayout('layouts.main', 'modulos.servicio_externo.index', [
            'titulo'      => 'Servicio Externo',
            'perm'        => $perm,
    ...
    public function searchAjax(): void
    {
        ...
        ob_start();
        if (empty($rows)) {
            echo '<tr><td colspan="6" class="text-center py-5 text-muted"><i class="bi bi-tools fs-3 d-block mb-2"></i>No se encontraron órdenes.</td></tr>';
        } else {
            foreach ($rows as $r) {
    ...
    public function enviarCorreoAjax(): void
    {
        ...
        try {
            ...
            $asunto = 'Orden Servicio Externo ' . ($numero !== '' ? $numero : '') . ($empresaNombre !== '' ? ' — ' . $empresaNombre : '');
            ...
            $enviado  = $emailSvc->enviarPdfSimple(
                ...
                'Orden_ServicioExterno_' . ($numero !== '' ? $numero : 'orden'), $empresaNombre
            );
    ...
    /**
     * Busca repuestos/servicios del catálogo con datos completos (precios de lista,
            ...
            $rows = array_map(function ($p) use ($repo, $repoInv, $idEmpresa, $idBodega, $idOrden) {
                ...
                if ($idBodega > 0 && $esInv) {
                    $stock = $repoInv->getStockActual(
                        ...
                        $idOrden > 0 ? 'servicioexterno_orden' : null
                    );
                }
    ...
    private static function filaHistorial(array $r): string
    {
        ...
        return '<tr class="se-row" role="button" tabindex="0" data-row=\'' . $dataJson . '\' onclick="seAbrirVer(this)">
                    ...
                    <td data-col="equipo" class="text-truncate" style="max-width:220px">' . htmlspecialchars($r['equipo_descripcion'] ?? '') . '</td>
                    <td data-col="cliente" class="text-truncate" style="max-width:220px">' . htmlspecialchars($r['cliente_nombre'] ?? '') . '</td>
