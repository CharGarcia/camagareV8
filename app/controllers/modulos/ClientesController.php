...
class ClientesController extends BaseModuloController
{
     ...
     * Además de tener permiso, la ficha solo pinta cada pestaña si el cliente TIENE datos
    ...
    /** Anticipos: el anticipo de un cliente se registra como un ingreso, así que ese es su permiso. */
    public const RUTAS_ANTICIPOS = ['modulos/ingresos'];
    ...
    public function transaccionesAjax(): void
    {
        ...
        $fuentes = $this->fuentesTransacciones();
        if (empty($fuentes)) {
    ...
    /**
     * Qué pestañas de consulta tiene sentido mostrar en la ficha de ESTE cliente: la ficha
     * las pinta solo si el usuario tiene permiso y además el cliente tiene datos, para no
     * ofrecer pestañas vacías. Son consultas de existencia (EXISTS sobre las mismas fuentes
     * que alimentan cada pestaña), así que salen baratas al abrir la ficha.
     */
    ...
    /**
     * Pestaña "Anticipos" de la ficha: saldo a favor del cliente y los movimientos que lo
     * forman, con la misma fórmula del saldo de una forma de cobro tipo ANTICIPO.
     */
    public function anticiposAjax(): void
    {
        ...
        if (!\App\Helpers\Permisos::puedeVerAlguna(self::RUTAS_ANTICIPOS)) {
            http_response_code(403);
            echo json_encode(['ok' => false, 'error' => 'No tiene permiso para ver los anticipos (Ingresos).']);
            exit;
        }
     ...
     * período que muestra la pestaña (misma fuente), con el cliente, el período y el
    ...
    public function estadoCuentaExcel(): void
    {
        ...
        try {
            ...
            $cliente = $this->service->getFicha($id, $idEmpresa) ?? [];
            ...
            $origenes = [
                ...
                'COBRO'         => 'Cobro',         'RETENCION' => 'Retención',
            ...
            $info    = [
                'Cliente'  => trim(($cliente['nombre'] ?? '') . '  ' . ($cliente['identificacion'] ?? '')),
                'Periodo'  => match (true) {
                ...
                'Resumen'  => sprintf(
                    'Saldo anterior %s  |  Cargos %s  |  Cobros %s  |  Retenciones y NC %s  |  Saldo %s',
