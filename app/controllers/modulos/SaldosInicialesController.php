...
class SaldosInicialesController extends BaseModuloController
{
        ...
        try {
            ...
            foreach ($hoja->getRowIterator() as $row) {
                ...
                $filas[] = [
                    ...
                    'ruc_cliente'      => $fila['RUC']               ?? $fila['RUC_CLIENTE'] ?? '',
                    'nombre_cliente'   => $fila['NOMBRE_CLIENTE']    ?? $fila['CLIENTE'] ?? '',
    ...
    public function descargarTemplateCxc(): void
    {
        ...
        $encabezados = [
            ...
            'RUC_CLIENTE', 'NOMBRE_CLIENTE', 'SALDO_PENDIENTE', 'OBSERVACIONES'
        ];
        $ejemplos = [
            '001-001-000001', '2024-01-01', '2024-01-31',
            '1234567890001', 'Empresa Ejemplo S.A.', '1500.00', 'Saldo migrado'
        ];
        ...
        $hoja->getStyle('A1:G1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        ...
        try {
            ...
            foreach ($hoja->getRowIterator() as $row) {
                ...
                $filas[] = [
                    ...
                    'fecha_emision'    => $this->parseDate($fila['FECHA_EMISION']     ?? ''),
                    'fecha_vencimiento'=> $this->parseDate($fila['FECHA_VENCIMIENTO'] ?? ''),
                    'ruc_proveedor'    => $fila['RUC']               ?? $fila['RUC_PROVEEDOR'] ?? '',
                    'nombre_proveedor' => $fila['NOMBRE_PROVEEDOR']  ?? $fila['PROVEEDOR'] ?? '',
                    'saldo_inicial'    => str_replace(',', '.', $fila['SALDO_PENDIENTE'] ?? $fila['SALDO'] ?? '0'),
                    'observaciones'    => $fila['OBSERVACIONES']     ?? '',
                ];
            }
    ...
    public function descargarTemplateCxp(): void
    {
        ...
        $encabezados = [
            ...
            'RUC_PROVEEDOR', 'NOMBRE_PROVEEDOR', 'SALDO_PENDIENTE', 'OBSERVACIONES'
        ];
        $ejemplos = [
            'FACTURA_COMPRA', '001-001-000001', '2024-01-01', '2024-01-31',
            '1234567890001', 'Proveedor Ejemplo S.A.', '2000.00', 'Saldo migrado'
        ];
