...
use App\Services\modulos\ProyectoService;

class ProyectosController extends BaseModuloController
{
    private ProyectoService $service;
    private const RUTA_MODULO = 'modulos/proyectos';

    public function __construct()
    {
        parent::__construct();
        $repository = new ProyectoRepository();
        $rules = new ProyectoRules();
        $logService = new LogSistemaService();
        $this->service = new ProyectoService($repository, $rules, $logService);
    }
    ...
    public function index(): void
    {
        ...
        $clientes = $clienteModel->getActivosPorEmpresa($idEmpresa);
        ...
        $this->viewWithLayout('layouts.main', 'modulos.proyectos.index', [
            'titulo'     => 'Proyectos',
            'perm'       => $perm,
    ...
    public function searchAjax(): void
    {
        ...
        if (empty($rows)) {
            ...
            foreach ($rows as $r) {
                ...
                echo '<tr class="proyecto-row" role="button" tabindex="0" data-row=\'' . $dataJson . '\' onclick="abrirModalEditar(this)">
                        ...
                        <td class="text-center" data-col="porcentaje_ejecucion">
                            <div class="progress" style="height: 6px; width: 60px; margin: 0 auto;" title="' . (int)($r['porcentaje_ejecucion'] ?? 0) . '%">
                                <div class="progress-bar" role="progressbar" style="width: ' . (int)($r['porcentaje_ejecucion'] ?? 0) . '%"></div>
                            </div>
                            <span class="small" style="font-size: 0.7rem;">' . (int)($r['porcentaje_ejecucion'] ?? 0) . '%</span>
                        </td>
    ...
    public function getDetalleAjax(): void
    {
        ...
        try {
            ...
            if (!$p) throw new \Exception('Proyecto no encontrado');
    ...
    public function store(): void
    {
        ...
        try {
            ...
            echo json_encode(['ok' => true, 'msg' => 'Proyecto creado correctamente.', 'id' => $id]);
        } catch (\Throwable $e) {
            echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
        }
        exit;
    ...
    public function update(): void
    {
        ...
        try {
            ...
            echo json_encode(['ok' => true, 'msg' => 'Proyecto actualizado correctamente.']);
        } catch (\Throwable $e) {
            echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
        }
        exit;
    ...
    public function delete(): void
    {
        ...
        try {
            ...
            echo json_encode(['ok' => true, 'msg' => 'Proyecto eliminado correctamente.']);
        } catch (\Throwable $e) {
            echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
        }
        exit;
                    ...
                    <tbody>
                        <?php foreach ($rows as $r): ?>
                            <tr>
                                ...
                                <td><?= number_format((float)($r['presupuesto'] ?? 0), 2) ?></td>
                                <td><?= ucfirst($r['estado'] ?? '—') ?></td>
                                <td><?= htmlspecialchars((string)($r['fecha_inicio'] ?? '—')) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<?php
            ...
            $html2pdf->output('Proyectos_' . date('Ymd_His') . '.pdf', 'D');
            exit;
        } catch (\Throwable $e) {
    ...
    public function exportExcel(): void
    {
        ...
        try {
            ...
            $headers = ['Código', 'Nombre', 'Cliente', 'Presupuesto', 'Ejecución %', 'Inicio', 'Fin', 'Estado'];
            $exportData = [];
            foreach ($rows as $r) {
                $exportData[] = [
                    (string)($r['codigo'] ?? ''),
                    ...
                    (float)($r['presupuesto'] ?? 0),
                    (float)($r['porcentaje_ejecucion'] ?? 0),
            ...
            $reportService->exportToExcel('Proyectos', $headers, $exportData, 'Listado Proyectos', $nombreEmpresa);
            exit;
        } catch (\Throwable $e) {
    ...
    private function recogerDatosFormulario(): array
    {
        return [
            ...
            'porcentaje_ejecucion' => (float)($_POST['porcentaje_ejecucion'] ?? 0),
