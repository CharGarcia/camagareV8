...
class RetornosCvController extends BaseModuloController
{
    ...
    /**
     * Pestaña "Detalles" del modal de filtros: búsqueda libre dentro de los retornos
     * (productos retornados con lote/NUP, bodega y consignación de origen). Devuelve
     * id/estado/serie/secuencial para abrir el modal del retorno (abrirModalRetornoVer).
     */
    public function buscarDetallesAjax(): void
    {
        ...
        foreach ($this->service->buscarEnDetalles($idEmpresa, $q, $idUsuarioFiltro, 50) as $r) {
            $rows[] = [
                ...
                'fecha'        => !empty($r['fecha_retorno']) ? date('d-m-Y', strtotime((string) $r['fecha_retorno'])) : '',
    ...
    /** Genera el Excel del retorno (mismas columnas que el PDF). */
    public function excel(): void
    {
        ...
        try {
            ...
            $sheet->setCellValue('C3', 'Cliente: ' . (string)($retorno['cliente_nombre'] ?? ''));
            $sheet->setCellValue('A4', 'Identificación: ' . (string)($retorno['cliente_identificacion'] ?? ''));
            $sheet->setCellValue('C4', 'Estado: ' . ucfirst((string)($retorno['estado'] ?? '')));
            ...
            $nombre = 'Retorno_' . ($numero !== '' ? $numero : 'comprobante') . '.xlsx';
