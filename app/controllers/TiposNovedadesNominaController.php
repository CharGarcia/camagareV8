...
class TiposNovedadesNominaController extends Controller
{
    ...
    /**
     * AJAX: listado de tipos de novedades (tabla), para búsqueda y
    ...
    {
        if (empty($rows)) {
            return '<tr><td colspan="4" class="text-center py-5 text-muted"><i class="bi bi-journal-plus fs-3 d-block mb-2"></i>No hay tipos de novedades registrados.</td></tr>';
        }
        $html = '';
        foreach ($rows as $r) {
            $id = (int) ($r['id'] ?? $r['id_tipos_novedades_nomina'] ?? 0);
            $status = (int) ($r['status'] ?? 1);
            $html .= '<tr class="tipo-novedad-row" role="button" tabindex="0" data-id="' . $id . '"'
            ...
            $html .= '<td class="text-end">'
                . '<form method="POST" action="' . BASE_URL . '/config/tiposNovedadesNominaDelete" class="d-inline" onsubmit="return confirm(&quot;¿Eliminar este tipo de novedad?&quot;);" onclick="event.stopPropagation();">'
