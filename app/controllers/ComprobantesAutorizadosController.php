...
class ComprobantesAutorizadosController extends Controller
{
    ...
    /**
     * AJAX: listado de comprobantes autorizados (tabla), para búsqueda y
    ...
    public function searchAjax(): void
    {
        ...
        $ordenCol = trim($_GET['sort'] ?? $_POST['sort'] ?? 'codigo_comprobante');
    ...
    {
        if (empty($rows)) {
            return '<tr><td colspan="4" class="text-center py-5 text-muted"><i class="bi bi-file-earmark-text fs-3 d-block mb-2"></i>No hay comprobantes autorizados registrados.</td></tr>';
        }
        $html = '';
        foreach ($rows as $r) {
            $id = (int) ($r['id'] ?? $r['id_comprobante'] ?? 0);
            $status = (int) ($r['status'] ?? 1);
            $html .= '<tr class="comprobante-row" role="button" tabindex="0" data-id="' . $id . '"'
                . ' data-codigo="' . htmlspecialchars($r['codigo_comprobante'] ?? '') . '"'
                . ' data-comprobante="' . htmlspecialchars($r['comprobante'] ?? '') . '"'
                . ' data-status="' . $status . '">';
            $html .= '<td><code>' . htmlspecialchars($r['codigo_comprobante'] ?? '') . '</code></td>';
            $html .= '<td>' . htmlspecialchars($r['comprobante'] ?? '') . '</td>';
            ...
            $html .= '<td class="text-end">'
                . '<form method="POST" action="' . BASE_URL . '/config/comprobantesAutorizadosDelete" class="d-inline" onsubmit="return confirm(&quot;¿Eliminar este comprobante?&quot;);" onclick="event.stopPropagation();">'
