...
class FormasPagoSriController extends Controller
{
    ...
    /**
     * AJAX: listado de formas de pago SRI (tabla), para búsqueda y
    ...
    {
        if (empty($rows)) {
            return '<tr><td colspan="4" class="text-center py-5 text-muted"><i class="bi bi-cash-stack fs-3 d-block mb-2"></i>No hay formas de pago registradas.</td></tr>';
        }
        $html = '';
        foreach ($rows as $r) {
            $id = (int) ($r['id'] ?? $r['id_forma_pago'] ?? 0);
            $status = (int) ($r['status'] ?? 1);
            $html .= '<tr class="forma-pago-row" role="button" tabindex="0" data-id="' . $id . '"'
            ...
            $html .= '<td class="text-end">'
                . '<form method="POST" action="' . BASE_URL . '/config/formasPagoSriDelete" class="d-inline" onsubmit="return confirm(&quot;¿Eliminar esta forma de pago?&quot;);" onclick="event.stopPropagation();">'
