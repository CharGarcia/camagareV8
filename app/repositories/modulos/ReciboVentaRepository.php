...
class ReciboVentaRepository extends BaseRepository
{
    ...
    /**
     * Series (establecimiento-puntoEmision) que REALMENTE tienen al menos un
     * recibo guardado, para poblar el filtro "Serie" del buscador. A propósito
     * NO usa los puntos de emisión configurados actualmente (esos solo sirven
     * para elegir la serie de un documento NUEVO).
     */
    ...
    public function getImpuestosPorDetalles(array $idsDetalle): array
    {
        ...
        $sql = "SELECT * FROM recibos_venta_detalle_impuestos WHERE id_recibo_detalle IN ($ph)";

        $porDetalle = [];
        foreach ($this->query($sql, $ids)->fetchAll() as $imp) {
            $porDetalle[(int) $imp['id_recibo_detalle']][] = $imp;
        }
        return $porDetalle;
    }
    ...
    public function crearServicioLibre(int $idEmpresa, int $idUsuario, string $nombre, float $precio, ?float $porcentajeIva = null, ?string $codigoPorcentaje = null, ?string $codigoDeseado = null): array
    {
        ...
        // insertar, por si cambió entre que se buscó y que se guardó el recibo.
