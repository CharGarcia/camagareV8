...
class CategoriaRepository extends BaseRepository
{
    ...
    /**
     * Devuelve el listado paginado y con búsqueda para Categorías.
     */
    public function getListado(
        ...
        if ($buscar !== '') {
            $whereSql .= " AND (c.nombre ILIKE :b)";
            $params[':b'] = '%' . $buscar . '%';
        }

        // Validación de columnas permitidas para order by
        $cols = ['nombre' => 'c.nombre', 'status' => 'c.status'];
        $col  = $cols[$ordenCol] ?? 'c.nombre';
        $dir  = ($ordenDir === 'DESC') ? 'DESC' : 'ASC';

        // 1. Contar total
        $sqlCount = "SELECT COUNT(*) FROM {$this->table} c {$whereSql}";
        ...
        $sqlRows = "SELECT c.id, c.nombre, c.status,
    ...
    /**
     * Obtiene el detalle de una categoría incluyendo nombres de auditoría.
     */
    public function getDetalleCompleto(int $id, int $idEmpresa): ?array
    {
        $sql = "SELECT c.*, 
                ...
                FROM {$this->table} c
                LEFT JOIN usuarios u_crea ON u_crea.id = c.created_by
                LEFT JOIN usuarios u_act ON u_act.id = c.updated_by
    ...
    /**
     * Cuenta cuántos productos activos tiene asignada la categoría.
     */
    public function contarProductosAsignados(int $id, int $idEmpresa): int
    {
        ...
        // NOTA: Ajustar si productos tiene borrado logico "eliminado=false" (el schema decía status y no "eliminado", me basaré en los que encuentre)
        $st = $this->db->prepare($sql);
        $st->execute([':id' => $id, ':id_empresa' => $idEmpresa]);
