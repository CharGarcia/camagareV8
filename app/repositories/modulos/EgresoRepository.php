...
class EgresoRepository extends BaseRepository
{
    ...
    /**
     * Conceptos de egreso activos, con la cuenta VIGENTE del concepto en Egresos
     * (`cuenta_id` / `cuenta_codigo` / `cuenta_nombre`): manda el asiento programado de la
     * naturaleza ('opcion_egreso') y la columna del módulo queda de respaldo. Es el mismo
     * COALESCE que usan el asiento (AsientoBuilderService::generarAsientoEgreso), Configuración
    ...
    public function getDocumentosPendientesEmpleado(int $idEmpleado, int $idEmpresa): array
    {
        $sql = "WITH pagado_rol AS (
                ...
                HAVING ROUND(SUM(n.valor) - COALESCE(pp.total_pagado, 0), 2) > 0
                UNION ALL
                ...
                WHERE dcd.id_empleado = :id_emp AND dcd.id_empresa = :id_empresa
                  ...
                  AND ROUND(dcd.valor - COALESCE(pdc.total_pagado, 0), 2) > 0
                UNION ALL
    ...
    public function getDocumentosPendientesProveedor(int $idProveedor, int $idEmpresa): array
    {
        ...
        $sql = "WITH pagado AS (
                ...
                WHERE c.id_proveedor = :id_prov
                  ...
                  AND ROUND(c.importe_total + COALESCE(c.total_terceros, 0) - COALESCE(p.total_pagado, 0) - COALESCE(nn.total_nc, 0) + COALESCE(nn.total_nd, 0), 2) > 0
                UNION ALL
        ...
        if ($tipo === 'COMPRA') {
            ...
            $sql = "WITH pagado AS (
                    ...
                    WHERE cb.id_empresa = :id_empresa
                      ...
                      AND ROUND(cb.importe_total + COALESCE(cb.total_terceros, 0) - COALESCE(p.total_pagado, 0) - COALESCE(rcp.total_retenido, 0) - COALESCE(nn.total_nc, 0) + COALESCE(nn.total_nd, 0), 2) > 0
                      $filtroBusq
