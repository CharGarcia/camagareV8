...
class DecimoTerceroRepository extends BaseRepository
{
    ...
    public function crearCabecera(array $d): int
    {
        ...
        // plazo legal, casi siempre futuro respecto al cálculo). Ver mismo comentario en
        // DecimoCuartoRepository::crearCabecera().
        $sql = "INSERT INTO {$this->table} (
                    id_empresa, anio, fecha_desde, fecha_hasta, fecha_limite_pago,
                    fecha_emision, base_calculo, estado, created_by, updated_by, created_at, updated_at, eliminado
                ) VALUES (
                    :id_empresa, :anio, :fecha_desde, :fecha_hasta, :fecha_limite,
                    CURRENT_DATE, :base_calculo, 'borrador', :id_u, :id_u, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, false
                )";
        $st = $this->db->prepare($sql);
        $st->execute([
    ...
    public function limpiarDetalle(int $idCabecera, array $excluirEmpleados = []): void
    {
        ...
        $sql = "DELETE FROM decimo_tercero_detalle WHERE id_cabecera = :id AND id_empleado NOT IN (" . implode(',', $ph) . ")";
        $st = $this->db->prepare($sql);
        $st->execute($params);
    }
    ...
    {
        $sql = "SELECT DISTINCT dtd.id_empleado
                FROM decimo_tercero_detalle dtd
                INNER JOIN egresos_detalle d ON d.id_referencia_documento = dtd.id
                INNER JOIN egresos_cabecera e ON e.id = d.id_egreso
                WHERE dtd.id_cabecera = :id AND d.tipo_documento = 'DECIMO_TERCERO'
    ...
    public function getEmpleadosActivos(int $idEmpresa): array
    {
        $sql = "SELECT id, identificacion, nombres_apellidos, sexo, decimo_tercero,
                ...
                WHERE id_empresa = :id_empresa AND eliminado = false AND estado = 'activo'
                  AND (decimo_tercero IS NULL OR decimo_tercero <> 'no_recibe')";
        $st = $this->db->prepare($sql);
