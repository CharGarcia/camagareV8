...
if (!function_exists('enviar_correo_invitacion_videollamada')) {
    /**
     * Invitación a una videollamada.
     *
     * Cada destinatario recibe SU propio enlace: para los invitados externos es
     * el enlace con token que les da acceso sin cuenta, y para los usuarios del
     * sistema es la dirección del módulo.
     *
     * @param array $data titulo, fecha_texto, anfitrion, codigo, enlace,
     *                    nombre_destinatario, empresa, es_invitado, descripcion
     */
    function enviar_correo_invitacion_videollamada(string $correoDestino, array $data, bool $esRecordatorio = false): bool
    {
        if (!filter_var($correoDestino, FILTER_VALIDATE_EMAIL)) {
            $GLOBALS['LAST_EMAIL_ERROR'] = 'Correo de destino inválido.';
            return false;
        }
        ...
        try {
            ...
            $mail->Subject = ($esRecordatorio ? 'Recordatorio: ' : 'Invitación: ')
                           . ($data['titulo'] ?? 'Videollamada');

            $data['es_recordatorio'] = $esRecordatorio;

            ob_start();
            require MVC_APP . '/views/emails/invitacion_videollamada.php';
            $mail->Body = ob_get_clean();
            $mail->isHTML(true);
            ...
            $GLOBALS['LAST_EMAIL_ERROR'] = $mail->ErrorInfo ?? $e->getMessage();
            error_log('Mailer Error (InvitacionVideollamada): ' . ($GLOBALS['LAST_EMAIL_ERROR']));
            return false;
        }
    }
}
...
if (!function_exists('enviar_correo_sri_pendientes')) {
     ...
     * SRI sin una resolución definitiva (autorizado/rechazado). Lo dispara el
     * cron (SriReintentosPendientesService::avisarSiCorresponde).
     *
     * @param array $data empresa_nombre, tipo_comprobante, id_comprobante,
     *                     clave_acceso, minutos
     */
    function enviar_correo_sri_pendientes(string $correoDestino, array $data): bool
    {
        if (!filter_var($correoDestino, FILTER_VALIDATE_EMAIL)) {
            $GLOBALS['LAST_EMAIL_ERROR'] = 'Correo de destino inválido.';
            return false;
        ...
        try {
            ...
            $mail->Subject = 'Comprobante sin resolver en el SRI hace ' . (int) ($data['minutos'] ?? 0) . ' minutos';

            ob_start();
            require MVC_APP . '/views/emails/sri_pendientes.php';
            $mail->Body = ob_get_clean();
            $mail->isHTML(true);
            ...
            $GLOBALS['LAST_EMAIL_ERROR'] = $mail->ErrorInfo ?? $e->getMessage();
            error_log('Mailer Error (SriPendientes): ' . ($GLOBALS['LAST_EMAIL_ERROR']));
            return false;
        }
    }
}

if (!function_exists('enviar_correo_firma_caducidad')) {
    /**
     * Avisa a la empresa que su firma electrónica está por caducar (o acaba de
     * caducar). Lo dispara el cron (FirmaCaducidadAvisoService::enviarAvisos).
     *
     * @param array $data empresa_nombre, empresa_ruc, fecha_expiracion (Y-m-d),
     *                     dias (negativo = caducada), url (opcional)
     */
    function enviar_correo_firma_caducidad(string $correoDestino, array $data): bool
    {
        if (!filter_var($correoDestino, FILTER_VALIDATE_EMAIL)) {
            $GLOBALS['LAST_EMAIL_ERROR'] = 'Correo de destino inválido.';
            return false;
        ...
        try {
            ...
            if ($dias > 1) {
                $asunto = 'Su firma electrónica caduca en ' . $dias . ' días';
            } elseif ($dias === 1) {
                $asunto = 'Su firma electrónica caduca mañana';
            } elseif ($dias === 0) {
                $asunto = 'Su firma electrónica caduca hoy';
            } else {
                $asunto = 'Su firma electrónica caducó: no podrá emitir comprobantes electrónicos';
            }
            $nombreEmp = trim((string) ($data['empresa_nombre'] ?? ''));
            $mail->Subject = $asunto . ($nombreEmp !== '' ? ' — ' . $nombreEmp : '');

            ob_start();
            require MVC_APP . '/views/emails/firma_caducidad.php';
            $mail->Body = ob_get_clean();
            $mail->isHTML(true);
            ...
            $GLOBALS['LAST_EMAIL_ERROR'] = $mail->ErrorInfo ?? $e->getMessage();
            error_log('Mailer Error (FirmaCaducidad): ' . ($GLOBALS['LAST_EMAIL_ERROR']));
            return false;
        }
    }
}
