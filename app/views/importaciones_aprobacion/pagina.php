...
<head>
    ...
    <title>Aprobación de importación</title>
    <style>
...
<div class="wrap">
    <div class="card">
        <?php if ($vista === 'detalle'): $c = $importacion; ?>
            <div class="card-head">
                <h1>Aprobación de importación</h1>
                <p><?= $e($c['empresa_nombre']) ?></p>
            </div>
            <div class="card-body">
                <div class="meta">
                    <div><div class="lbl">N°</div><div class="val"><?= $e($c['numero_importacion']) ?></div></div>
                    <div><div class="lbl">Proveedor</div><div class="val"><?= $e($c['proveedor_nombre']) ?></div></div>
                    <div><div class="lbl">Costo nacionalizado</div><div class="val">$ <?= number_format((float) $c['costo_total_nacionalizado'], 2) ?></div></div>
                    <div><div class="lbl">Líneas</div><div class="val"><?= count($c['detalles'] ?? []) ?></div></div>
                    <div><div class="lbl">Solicitada por</div><div class="val"><?= $e($c['creado_por_nombre']) ?></div></div>
                </div>

                <table>
                    <thead><tr><th>Producto</th><th class="num">Cantidad</th><th class="num">Costo unit.</th><th class="num">Costo total</th></tr></thead>
                    <tbody>
                        <?php foreach (($c['detalles'] ?? []) as $d): ?>
                            <tr>
                                <td><?= $e($d['descripcion'] ?: $d['codigo_producto_raw']) ?></td>
                                <td class="num"><?= $e((float) $d['cantidad']) ?></td>
                                <td class="num">$ <?= number_format((float) $d['costo_unitario_nacionalizado'], 4) ?></td>
                                <td class="num">$ <?= number_format((float) $d['costo_total_nacionalizado'], 2) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <div class="actions">
                    <form method="POST" action="<?= $base ?>/aprobar-importacion/<?= $e($token) ?>/aprobar" onsubmit="return confirm('¿Aprobar esta importación? Se postearán las entradas al kardex y se generará el asiento contable.');">
                ...
                <form method="POST" action="<?= $base ?>/aprobar-importacion/<?= $e($token) ?>/rechazar" class="rechazo-box" id="rbox">
                    <input type="hidden" name="token" value="<?= $e($token) ?>">
                    <label style="font-size:13px;font-weight:600;color:#b91c1c;">Motivo del rechazo</label>
                    <textarea name="motivo" rows="3" required placeholder="Explique por qué rechaza esta importación…"></textarea>
                    <div style="margin-top:10px;"><button type="submit" class="btn btn-no">Confirmar rechazo</button></div>
                </form>

                <p class="muted">El inventario y el asiento contable se generan únicamente cuando la importación es aprobada.</p>
            </div>
        <?php else: /* resultado */
