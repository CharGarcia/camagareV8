...
$vistaConfig = PreferenciasHelper::getPreferenciasVista($rutaModulo . '/marcaciones');
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
    ...
    <a href="<?= $urlBase ?>" class="btn btn-outline-secondary btn-sm px-3"><i class="bi bi-geo-alt-fill me-1"></i> Puntos de servicio</a>
</div>
...
<div class="card cmg-table-card border-0 shadow-sm rounded-3">
    <div class="card-header bg-white py-2 px-3 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="d-flex align-items-center gap-2">
            ...
            <div id="fbBuscadorCASISM" style="width: 460px;"></div>
            <input type="hidden" id="buscarCasisM" value="<?= htmlspecialchars($buscar) ?>">
            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    if (!window.FiltrosBusqueda) return;
                    new FiltrosBusqueda({
                        containerId: 'fbBuscadorCASISM',
                        hiddenInputId: 'buscarCasisM',
                        fields: [
            ...
            <div class="btn-group btn-group-sm">
                ...
                ?>
                <?= PreferenciasHelper::renderDropdownColumnas($columnasTabla, $vistaConfig, $rutaModulo . '/marcaciones') ?>
            </div>
        </div>
...
<script>
    (function () {
        ...
        const inputB = document.getElementById('buscarCasisM');
        ...
        async function cargarListado(page = 1) {
            ...
            const uri = `${urlBase}/marcacionesSearchAjax?b=${encodeURIComponent(b)}&page=${page}&sort=${currentSort}&dir=${currentDir}`;
            try {
        ...
        window.eliminarMarcacion = function (id) {
            if (!window.Swal) { if (!confirm('¿Eliminar esta marcación?')) return; }
            const doDelete = () => {
                const fd = new FormData(); fd.append('id_eliminar', id);
                fetch(`${urlBase}/deleteMarcacion`, { method: 'POST', body: fd })
                    .then(r => r.json())
                    .then(j => {
        ...
        if (window.CMG_initSort) {
            window.CMG_initSort('control_asistencia_marc', (col, dir) => {
