...
<div class="card cmg-table-card w-100 border-0 shadow-sm rounded-3">
    <div class="card-header bg-white py-2 px-3 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="d-flex align-items-center gap-2 flex-wrap">
            ...
            // Las claves (key) deben existir en los mapas de NotaDebitoRepository::getListado().
            ...
            // Dos pestañas: "Nota de débito" (filtros por campo de la cabecera) y "Detalles"
            ...
            //   Documento modificado: [Nº documento 4][Fecha del documento 5][Motivo 3]
            ...
            $filtrosNotasDebito = [
                ...
                ['tab' => $tN, 'key' => 'iva',      'label' => 'IVA',      'icon' => 'bi-percent',         'type' => 'number_range', 'grupo' => 'Valores', 'col' => 4],
                // ── Cliente ──
            ...
            <div id="fmBuscadorND"></div>
            <input type="hidden" id="buscarND" value="<?= htmlspecialchars($buscar) ?>">
            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    if (!window.FiltrosModal) return;
                    new FiltrosModal({
                        containerId: 'fmBuscadorND',
                        hiddenInputId: 'buscarND',
                        ...
                        busquedaDetalle: {
                            ...
                            onOpen: (row, fm) => {
                                fm.hide();
                                setTimeout(() => window.ND_abrirModalND({ dataset: { row: JSON.stringify({ id: row.id_nota, estado: row.estado_valor }) } }), 350);
                            },
                        },
                        fields: <?= json_encode($filtrosNotasDebito, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS) ?>,
                        loadingTarget: '#nd-table-body',   // se atenúa mientras se busca
                        onApply: () => window.ND_fetchSearch && window.ND_fetchSearch(1),
                    }).init();
                });
            </script>

            <?php // FiltrosModal (extraId) mueve estos botones dentro del grupo del buscador; si el JS no corre, quedan aquí. ?>
            <div id="fmExtraND" class="btn-group btn-group-sm">
                <?php
                $columnasTabla = [
                    'numero'              => 'Nº Nota',
                    'fecha_emision'       => 'Fecha',
        ...
        <div class="d-flex align-items-center gap-3">
            <span id="nd-pagination-info" class="text-muted small fw-medium"><?= $from ?>-<?= $to ?>/<?= $total ?></span>
            <div id="nd-pagination" class="btn-group btn-group-sm">
                <button type="button" class="btn btn-outline-secondary" <?= $page <= 1 ? 'disabled' : '' ?> onclick="window.ND_cambiarPaginaAjax(<?= $page - 1 ?>)"><i class="bi bi-chevron-left"></i></button>
                <button type="button" class="btn btn-outline-secondary" <?= $page >= $totalPages ? 'disabled' : '' ?> onclick="window.ND_cambiarPaginaAjax(<?= $page + 1 ?>)"><i class="bi bi-chevron-right"></i></button>
            </div>
        </div>
    </div>

    <div class="card-body p-0">
        <div class="nd-scroll w-100">
            <table class="table table-hover table-sm mb-0">
                <thead class="table-light">
                    <tr>
                        <th class="ps-3 sortable-header" role="button" onclick="window.ND_ordenar('secuencial')" data-col="numero">
                        ...
                        <th class="sortable-header" role="button" onclick="window.ND_ordenar('fecha_emision')" data-col="fecha_emision">
                        ...
                        <th class="sortable-header" role="button" onclick="window.ND_ordenar('cliente_nombre')" data-col="cliente_nombre">
                        ...
                        <th class="sortable-header" role="button" onclick="window.ND_ordenar('cliente_ruc')" data-col="cliente_ruc">
                        ...
                        <th class="sortable-header text-end" role="button" onclick="window.ND_ordenar('total_sin_impuestos')" data-col="total_sin_impuestos">
                        ...
                        <th class="sortable-header text-end" role="button" onclick="window.ND_ordenar('importe_total')" data-col="importe_total">
                        ...
                        <th class="sortable-header" role="button" onclick="window.ND_ordenar('usuario_nombre')" data-col="usuario_nombre">
                        ...
                        <th class="text-center sortable-header" role="button" onclick="window.ND_ordenar('estado_correo')" data-col="estado_correo">
                        ...
                        <th class="text-center pe-3 sortable-header" role="button" onclick="window.ND_ordenar('estado')" data-col="estado">
                ...
                <tbody id="nd-table-body">
                    <?php if (empty($rows)): ?>
                        <tr>
                            <td colspan="10" class="text-center py-5 text-muted"><i class="bi bi-file-earmark-minus fs-3 d-block mb-2"></i>No se encontraron notas de débito.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($rows as $r): ?>
                            ...
                            ?>
                            <tr class="nd-row" role="button" tabindex="0" data-row='<?= htmlspecialchars(json_encode($r), ENT_QUOTES, 'UTF-8') ?>' onclick="window.ND_abrirModalND(this)">
...
<script>
    ...
    window.currentDir   = window.ND_ORDEN_DIR;
</script>
<script src="<?= rtrim($base, '/') ?>/js/modulos/asiento_contable_tab.js?v=<?= asset_ver('/js/modulos/asiento_contable_tab.js') ?>" defer></script>
<script src="<?= rtrim($base, '/') ?>/js/modulos/nota_debito.js?v=<?= asset_ver('/js/modulos/nota_debito.js') ?>" defer></script>
