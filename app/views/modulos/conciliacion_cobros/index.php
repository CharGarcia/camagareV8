...
<div class="container-fluid pt-2 pb-3 px-0 px-md-3" id="modulo-conciliacion_cobros">
    ...
    <!-- ── Paso 1: Cuenta, punto de emisión, formato del banco y archivo ── -->
                ...
                <div class="col-md-3">
                    ...
                    <select id="cc-forma" class="form-select form-select-sm shadow-none" required>
                        ...
                        <?php foreach ($cuentas as $c): ?>
                            <option value="<?= (int) $c['id'] ?>" data-banco="<?= (int) ($c['id_banco'] ?? 0) ?>">
...
<script>
    ...
    window.CC_PERFILES = <?= json_encode(array_map(fn ($p) => [
        'id' => (int) $p['id'],
        'id_banco' => $p['id_banco'] !== null ? (int) $p['id_banco'] : null,
        'nombre_perfil' => $p['nombre_perfil'],
        'nombre_banco' => $p['nombre_banco'],
        'tipo_archivo' => $p['tipo_archivo'],
    ], $perfiles), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG) ?>;
</script>
