...
<style>
    .dc-scroll { max-height: calc(100dvh - 250px); overflow-y: auto; }
    .dc-scroll thead th { position: sticky; top: 0; z-index: 10; background: #f8f9fa; }
    .dc-row { cursor: pointer; }
    .dc-row:hover { background-color: rgba(0, 0, 0, .04); }
</style>
...
<div class="card cmg-table-card border-0 shadow-sm rounded-3">
    <div class="card-header bg-white py-2 px-3 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="d-flex align-items-center gap-2">
            <input type="text" id="buscarDc" class="form-control form-control-sm" style="width: 260px;"
                   placeholder="Buscar por año, región o estado..." value="<?= htmlspecialchars($buscar) ?>"
                ...
                <tbody id="tbodyDecimoCuarto">
                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $row):
                            ...
                            $region = $row['region_grupo'] === 'sierra_amazonia' ? 'Sierra / Amazonía' : 'Costa / Insular';
                        ?>
                            <tr class="dc-row" onclick="abrirModalVer(this)" data-row='<?= htmlspecialchars(json_encode($row), ENT_QUOTES) ?>'>
                                <td class="ps-3 fw-medium" data-col="anio"><?= (int) $row['anio'] ?></td>
                                <td data-col="region"><?= htmlspecialchars($region) ?></td>
...
<script>window.BASE_URL = '<?= $base ?>'; window.DC_ANIO_ACTUAL = <?= $anioActual ?>;</script>
<?php include 'modal_calcular.php'; ?>
<?php include 'modal_detalle.php'; ?>
<script src="<?= $base ?>/js/modulos/decimo_cuarto.js?v=<?= asset_ver('/js/modulos/decimo_cuarto.js') ?>"></script>

<script>
    (function () {
        'use strict';
        const urlBase = '<?= $urlBase ?>';
        const inputB = document.getElementById('buscarDc');
        ...
        async function cargarListado(page = 1) {
            ...
            try {
                ...
                if (data.ok) {
                    ...
                    document.getElementById('tbodyDecimoCuarto').innerHTML = data.rows;
        ...
        if (window.CMG_initSort) {
            window.CMG_initSort('decimo_cuarto', (col, dir) => { currentSort = col; currentDir = dir; cargarListado(1); }, { col: currentSort, dir: currentDir });
        }
        window.addEventListener('decimoCuartoActualizado', () => cargarListado(window.currentPage || 1));
    })();
</script>
