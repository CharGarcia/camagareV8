                ...
                <form id="formFacturaModal">
                    ...
                    <div class="d-flex align-items-center bg-light px-3 pt-2">
                        ...
                        <div class="ms-auto pb-1">
                            <?php
                            $pestanasConfig = ['tab-fv-pagos-btn' => 'Pagos'];
...
<script>
    ...
    async function guardarFactura() {
        ...
        // desde el recibo (procesarAsientoContable); el que se ve en la pestaña es de solo
        ...
        try {
            ...
            if (json.ok) {
                ...
                Swal.fire({
                    ...
                    position: 'top-end',
                    icon: 'success',
                    title: esActualizacion ? 'Recibo actualizado con éxito' : 'Recibo guardado con éxito',
    ...
    async function imprimirTicket() {
        ...
        try {
            ...
            // subtotal (ver ReciboVentaPdfService) — así la tirilla nunca muestra un
        ...
        try {
            ...
            if (json.ok) {
                ...
                if (json.data && json.data.length > 0) {
                    ...
                    // guardada (recibos migrados/históricos). Se inyecta como opción lo que falte
                    ...
                    if (selLote && currentLote && currentLote !== 'sin_lote') {
                        ...
                        if (selCad && currentCad) {
                            if (!Array.from(selCad.options).some(o => o.value === currentCad)) {
                                const optCad = new Option(rvFechaCadTexto(currentCad), currentCad);
                ...
                // Sin stock actual pero con lote guardado (recibo migrado/histórico):
    ...
    window.abrirModalFacturaVer = async function(row) {
        ...
        try {
            ...
            json.detalles.forEach(d => {
                ...
                // líneas inventariables cuando el lote NO es obligatorio (ver ReciboVentaService),
                // así que no cuentan como "dato real" para el fallback de abajo.
...
<script>
    ...
    async function abrirModalNuevaNCDesdeFV() {
        ...
        if (typeof window.NC_abrirModalNuevo === 'function') {
            ...
            setTimeout(() => {
                // Vendedor del recibo (no el que tenga hoy el cliente).
                window.NC_seleccionarCliente({ id: idCliente, nombre: nombreCliente, identificacion: rucCliente, id_vendedor: document.getElementById('m-select-vendedor')?.value || '' });
    ...
    // del recibo y —con permiso de actualizar en Asientos Contables— permite corregirlo y
    // guardarlo, comprobando que siga cuadrando con el total del recibo (el cuadre se mide
    // sobre la línea de cuenta por cobrar, no sobre el Debe total, que además lleva el costo de
    // ventas y el descuento). Mientras el recibo no tenga asiento se muestra la vista previa
    ...
    let _rvAsientoTab = null;
    function rvAsientoTab() {
        ...
        if (!_rvAsientoTab && typeof window.crearAsientoTab === 'function') {
            _rvAsientoTab = window.crearAsientoTab({
                prefijo: 'fv',
                moduloOrigen: 'recibo_venta',
                ...
                // La vista previa se calcula con los importes que hay en pantalla, no solo con
                // el id: así refleja lo que se está editando antes de guardar.
                previewParams: (forzada) => {
                    ...
                    return {
                        id_venta: forzada ? 0 : (window.RV_ID_ACTIVO || 0),
        ...
        return _rvAsientoTab;
    }

    // Refresca el asiento con los valores actuales del recibo (debounced para no spamear).
    // Se llama desde calcTotales() cada vez que cambian productos, precios o cantidades: como
    // aún se está editando el recibo, siempre se pide la VISTA PREVIA.
    window.fvDebouncedRecalcularAsiento = debounce(function() {
        const tab = rvAsientoTab();
        if (tab) tab.cargar(window.RV_ID_ACTIVO || 0, { vistaPrevia: true });
    }, 600);
...
window.fvEnviarCobroTarjeta = async function() {
    ...
    var result = await Swal.fire({
        ...
        html: '<p class="text-muted small mb-3">Se enviará al cliente un enlace para pagar <strong>$' + monto.toFixed(2) + '</strong> con Payphone de forma segura, junto con el PDF del recibo.</p>'
