...
/**
 * Lógica de negocio del módulo Servicio Externo.
 *
 * Una orden registra un servicio de mantenimiento realizado en el sitio del
 * cliente, con sus repuestos/mano de obra. La orden mueve inventario (salida al
 * guardar) pero NO genera asiento contable por sí misma: eso ocurre al emitir
 * el documento de venta (Factura o Recibo) desde generarDocumento().
 */
class ServicioExternoService
{
    private ServicioExternoRepository $repository;
    private ServicioExternoRules $rules;
    private LogSistemaService $logService;
    private ?InventarioService $inventarioService = null;

    /** Tipo de referencia en el kardex para los movimientos de la orden. */
    private const REF_TIPO = 'servicioexterno_orden';

    public function __construct(
        ServicioExternoRepository $repository,
        ServicioExternoRules $rules,
     ...
     * Solo productos del catálogo (la mano de obra/servicio libre no mueve
     * stock); el propio motor ignora los no inventariables.
     */
    ...
    /**
     * Registra la SALIDA de inventario de la orden (los repuestos se consumen al
    ...
    private function aplicarSalidaInventario(int $idOrden, int $idEmpresa, int $idUsuario, array $detalles, int $idEstablecimiento, int $idBodega, string $numeroOrden, bool $esEdicion): void
    {
        ...
        $this->getInventarioService()->procesarSalidaPorVenta(
            ...
            'Servicio Externo # ' . $numeroOrden, $esEdicion, self::REF_TIPO
        );
    }
     ...
     * (inventariables y no-servicio). La mano de obra e ítems libres no la requieren.
    ...
    /** Búsqueda libre dentro de las órdenes (servicios/productos) — pestaña Detalles del buscador. */
    ...
    public function crear(array $data): int
    {
        ...
        try {
            ...
            $cabecera = [
                ...
                'equipo_descripcion'  => trim((string) $data['equipo_descripcion']),
                'equipo_marca'        => $data['equipo_marca'] ?? null,
                'equipo_modelo'       => $data['equipo_modelo'] ?? null,
                'equipo_serie'        => $data['equipo_serie'] ?? null,
            ...
            // Salida de inventario: los repuestos se consumen al realizar el servicio.
            ...
            $this->logService->registrar($idUsuario, $idEmpresa, 'CREAR_ORDEN_SERVICIO_EXTERNO', 'servicioexterno_ordenes', $idOrden, null, $cabecera);
    ...
    public function actualizar(int $id, int $idEmpresa, array $data): void
    {
        ...
        try {
            ...
            $this->logService->registrar($idUsuario, $idEmpresa, 'ACTUALIZAR_ORDEN_SERVICIO_EXTERNO', 'servicioexterno_ordenes', $id, $cab, $data);
    ...
    public function cambiarEstado(int $id, int $idEmpresa, int $idUsuario, string $nuevoEstado): void
    {
        ...
        try {
            ...
            $this->logService->registrar($idUsuario, $idEmpresa, 'CAMBIAR_ESTADO_ORDEN_SERVICIO_EXTERNO', 'servicioexterno_ordenes', $id,
    ...
    public function eliminar(int $id, int $idEmpresa, int $idUsuario): void
    {
        ...
        try {
            ...
            $this->logService->registrar($idUsuario, $idEmpresa, 'ELIMINAR_ORDEN_SERVICIO_EXTERNO', 'servicioexterno_ordenes', $id, $cab, null);
            $db->commit();
        } catch (Exception $e) {
            if ($db->inTransaction()) $db->rollBack();
            throw $e;
    ...
    public function generarDocumento(int $idOrden, int $idEmpresa, int $idUsuario, string $tipo, array $extra, array $empresaConfig): array
    {
        ...
        if (empty($detalles)) {
            throw new Exception('La orden no tiene repuestos ni servicios.');
        }
        ...
        $payload = [
            ...
            'dias_credito'        => 0,
            'moneda'              => 'DOLAR',
            'observaciones'       => 'Generado desde orden de servicio externo ' . ($orden['numero_orden'] ?? ''),
        ...
        try {
            ...
            try {
                ...
                error_log("[ServicioExterno] No se pudo restaurar el inventario de la orden {$idOrden}: " . $e2->getMessage());
            }
            throw $e;
        }
        ...
        try {
            ...
            $this->logService->registrar($idUsuario, $idEmpresa, 'GENERAR_DOCUMENTO_SERVICIO_EXTERNO', 'servicioexterno_ordenes', $idOrden,
    ...
    /**
     * Inserta las líneas de detalle de una orden y devuelve los totales.
