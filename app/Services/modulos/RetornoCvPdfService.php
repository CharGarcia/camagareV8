...
class RetornoCvPdfService
{
    ...
    private function dibujarTablaDetalle(array $detalles, float $y): float
    {
        ...
        foreach ($detalles as $d) {
            ...
            // como aquí se hace un SetXY por COLUMNA, un retorno largo acababa generando
            // una página casi vacía por celda (60 líneas producían 74 páginas).
            if ($yRow + $h > $limiteY) {
                $pdf->AddPage();
            ...
            foreach ($cols as $i => $c) {
                ...
                if ($i === $descIdx) {
                    ...
                    // stretch = 1: un lote o NUP más largo que su columna se condensa
    ...
    private function dibujarFirmas(array $c, float $y): void
    {
        ...
        // Alto del bloque: 24 mm hasta la línea de firma más el nombre debajo. Si no
        // cabe entero, las firmas pasan a una hoja nueva; antes se clavaban en 272 mm
        // y terminaban dibujándose encima del listado.
        if ($y + 34 > $pdf->getPageHeight() - $pdf->getBreakMargin()) {
            $pdf->AddPage();
            $y = $pdf->GetY();
        }
        $yLinea = $y + 24;
