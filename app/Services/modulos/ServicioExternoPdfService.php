...
/**
 * PDF de la Orden de Servicio Externo (A4 vertical).
 *
 * Clon de OrdenCarWashPdfService: logo + datos de empresa a la izquierda y una
 * caja con el título, N.° y estado a la derecha. Luego: datos del equipo/cliente
 * y dirección del servicio, tabla de repuestos/mano de obra, totales + monto en
 * letras, información adicional y firmas.
 */
class ServicioExternoPdfService
{
    private TCPDF $pdf;
    ...
    public function generar(array $orden, array $empresa, string $outputDest = 'I')
    {
        ...
        $this->pdf->SetTitle('Orden Servicio Externo ' . ($orden['numero_orden'] ?? ''));
        ...
        $nombre = 'Orden_ServicioExterno_' . (($orden['numero_orden'] ?? '') !== '' ? $orden['numero_orden'] : 'orden') . '.pdf';
        if ($outputDest === 'S') {
    ...
    private function dibujarEncabezado(array $empresa, array $orden): float
    {
        ...
        $pdf->Cell($derW, 5, 'SERVICIO EXTERNO', 0, 1, 'C');
    ...
    // ─── Datos del equipo / cliente / dirección ───────────────────────────────
    private function dibujarDatosOrden(array $orden, float $y): float
    {
        ...
        $fecha = '';
        if (!empty($orden['fecha_servicio'])) {
            $ts = strtotime((string)$orden['fecha_servicio']);
            $fecha = $ts ? date('d/m/Y H:i', $ts) : (string)$orden['fecha_servicio'];
        }
        $equipo = trim((string)($orden['equipo_descripcion'] ?? '') . '  ' . (string)($orden['equipo_marca'] ?? '') . ' ' . (string)($orden['equipo_modelo'] ?? ''));
        $cliente  = trim((string)($orden['cliente_nombre'] ?? ''));
        $direccion = (string)($orden['direccion_servicio'] ?? '');
        ...
        $pdf->SetXY($mL + 2, $y + 2);
        $lbl('Equipo:'); $val($this->ajustarTexto($equipo !== '' ? $equipo : '—', 88), 88);
        $pdf->SetFont('helvetica', 'B', 8); $pdf->Cell(18, 5, 'Fecha:', 0, 0, 'R');
        $val($fecha, 0, 1);

        $pdf->SetXY($mL + 2, $pdf->GetY());
        $lbl('Cliente:'); $val($this->ajustarTexto($cliente !== '' ? $cliente : '—', 88), 88);
        $pdf->SetFont('helvetica', 'B', 8); $pdf->Cell(18, 5, 'Ident.:', 0, 0, 'R');
        $val((string)($orden['cliente_identificacion'] ?? ''), 0, 1);

        $pdf->SetXY($mL + 2, $pdf->GetY());
        $lbl('Dirección:'); $val($this->ajustarTexto($direccion !== '' ? $direccion : '—', $w - 30), $w - 30, 1);

        return $y + $boxH;
    }

    // ─── Tabla de repuestos / mano de obra ────────────────────────────────────
    private function dibujarTablaDetalle(array $detalles, float $y): float
    {
        ...
        if (empty($detalles)) {
            $pdf->SetX($mL);
            $pdf->Cell($this->contentW, 6, 'Sin repuestos ni servicios.', 1, 1, 'C');
        }
        $alt = false;
